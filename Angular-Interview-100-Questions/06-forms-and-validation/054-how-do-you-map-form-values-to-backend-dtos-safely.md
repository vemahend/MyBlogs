...
Safely mapping form values to backend DTOs means explicitly selecting, normalizing, and converting fields instead of casting the raw form value. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Map, do not cast. Similar-looking objects can have different trust and null rules.**

## 1. What problem does it solve?

Safely mapping form values to backend DTOs means explicitly selecting, normalizing, and converting fields instead of casting the raw form value.
...
Map, do not cast. Similar-looking objects can have different trust and null rules. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Disabled controls are omitted from `.value` but included by `getRawValue`
2. nullable controls affect types
3. date/decimal representations need deliberate conversion. A mapper creates a boundary object and prevents UI-only state leaking.
...
Map account ID, beneficiary ID, decimal amount, currency, reference, and idempotency key. Never send display labels, permission flags, or server-calculated fees as trusted values.
...
```ts
function toRequest(form:TransferForm):CreatePaymentRequest { const v=form.getRawValue(); return {fromAccountId:v.from.id,amount:v.amount.toFixed(2),currency:'NZD'}; }
```
...
**Use it when:** Always at non-trivial API boundaries, preferably with typed forms and mapper tests.

**Avoid or reconsider it when:** Avoid generic automapping when fields have security, money, or semantic differences.
...
Form model serves editing; request DTO serves transport; domain command serves server behavior. Similar shapes are not the same responsibility.
...
- `as PaymentRequest`
- sending disabled-but-sensitive fields accidentally
- floating-point money
- timezone conversion
- API DTO used directly as form model.
...
A payment screen uses **How do you map form values to backend DTOs safely**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Safely mapping form values to backend DTOs means explicitly selecting, normalizing, and converting fields instead of casting the raw form value.
- **Remember:** Map, do not cast. Similar-looking objects can have different trust and null rules.
