...
Dynamic forms add/remove controls or groups at runtime, usually with `FormArray`, based on user choices or schema-driven requirements. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Dynamic UI still needs a stable, typed form model.**

## 1. What problem does it solve?

Dynamic forms add/remove controls or groups at runtime, usually with `FormArray`, based on user choices or schema-driven requirements.
...
Dynamic UI still needs a stable, typed form model. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. The form model remains the source of truth. Each repeated item is a FormGroup with validators. Adding/removing controls changes value/status streams and template iteration must track stable identity.
...
A bulk-payment screen maintains a FormArray of recipients, each with account, amount, reference, and validation state; totals derive from the array.
...
```ts
rows=this.fb.array<FormGroup<RecipientForm>>([]);
add(){this.rows.push(this.createRecipient())}
...
**Use it when:** For repeated or conditional editable sections with validation.

**Avoid or reconsider it when:** For read-only tables or extremely large datasets better handled through row-level editing/paging.
...
FormArray is ordered repeated controls; FormGroup is named structure; FormRecord suits dynamic keys. A data grid is not automatically a form.
...
- Using indexes as durable IDs
- recreating controls on every render
- subscriptions per row leaking
- huge arrays without virtualization
- DTO mapping by blind cast.
...
A payment screen uses **How do you handle dynamic form fields**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Dynamic forms add/remove controls or groups at runtime, usually with `FormArray`, based on user choices or schema-driven requirements.
- **Remember:** Dynamic UI still needs a stable, typed form model.
