...
Reset a form deliberately after confirmed success, choosing whether to clear values, restore defaults, and reset dirty/touched/submitted state. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Success may clear; failure should usually preserve.**

## 1. What problem does it solve?

Reset a form deliberately after confirmed success, choosing whether to clear values, restore defaults, and reset dirty/touched/submitted state.
...
Success may clear; failure should usually preserve. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. `form.reset(defaults)` updates values and interaction state, but disabled controls and custom controls need correct handling. Do not reset before the server confirms success or the user loses recoverable input.
...
After a scheduled payment is confirmed, reset amount/reference while retaining the selected source account only if product requirements say so. On failure, preserve the draft.
...
```ts
this.api.create(request).subscribe({next:()=>this.form.reset({amount:0,reference:''}),error:()=>this.error.set('Not created; details preserved.')});
```
...
**Use it when:** After confirmed completion, cancellation, or explicit “new form” action.

**Avoid or reconsider it when:** Do not reset on recoverable errors or navigation if draft recovery is expected.
...
Reset means new editing session; patchValue changes selected fields; markAsPristine changes metadata only; rebuilding the form replaces controls/subscriptions.
...
- Reset on click before response
- unexpected nulls in non-nullable form
- stale server errors
- custom ControlValueAccessor not handling writeValue
...
A payment screen uses **How do you reset a form after submit**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Reset a form deliberately after confirmed success, choosing whether to clear values, restore defaults, and reset dirty/touched/submitted state.
- **Remember:** Success may clear; failure should usually preserve.
