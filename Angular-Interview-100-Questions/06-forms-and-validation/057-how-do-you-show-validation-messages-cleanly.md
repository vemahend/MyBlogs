...
Clean validation messages appear when useful, are accessible, and map error keys to consistent human language without cluttering the template. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Show the right message at the right time—not every message all the time.**

## 1. What problem does it solve?

Clean validation messages appear when useful, are accessible, and map error keys to consistent human language without cluttering the template.
...
Show the right message at the right time—not every message all the time. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Controls expose invalid, dirty, touched, pending, and errors. A reusable error component or mapping function renders the highest-priority relevant message and connects it with `aria-describedby`.
...
Show “Amount must be greater than zero” after touch or submit; show “Checking beneficiary…” while pending; map API domain errors separately from field validators.
...
```ts
@if(control.pending){<span>Checking…</span>} @else if(control.touched&&control.hasError('required')){<span id="amount-error">Amount is required</span>}
```
...
**Use it when:** For every user-editable form with a consistent interaction policy.

**Avoid or reconsider it when:** Do not hide server failures inside a generic field error when action-level feedback is needed.
...
Client field validation guides input; server validation handles authoritative/domain conflicts; summary errors help accessibility and long forms.
...
- Showing errors before interaction
- repeated if blocks everywhere
- server error attached to wrong control
- no error summary/focus
...
A payment screen uses **How do you show validation messages cleanly**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Clean validation messages appear when useful, are accessible, and map error keys to consistent human language without cluttering the template.
- **Remember:** Show the right message at the right time—not every message all the time.
