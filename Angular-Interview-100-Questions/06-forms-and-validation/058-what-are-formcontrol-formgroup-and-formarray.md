...
FormControl represents one value/status, FormGroup represents named controls, and FormArray represents an ordered collection of controls. Together they form a reactive form tree. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Control = field, Group = object, Array = repeated list.**

## 1. What problem does it solve?

FormControl represents one value/status, FormGroup represents named controls, and FormArray represents an ordered collection of controls. Together they form a reactive form tree.
...
Control = field, Group = object, Array = repeated list. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Value and validity aggregate upward: a group/array becomes invalid or pending based on children. Events propagate unless suppressed. Validators can exist at each level.
...
A transfer form is a FormGroup; amount is a FormControl; split-payment recipients are a FormArray of FormGroups.
...
```ts
form=this.fb.nonNullable.group({source:['',Validators.required],amount:[0,Validators.min(.01)],recipients:this.fb.array<FormGroup<RecipientForm>>([])});
```
...
**Use it when:** For explicit, testable form models, particularly complex workflows.

**Avoid or reconsider it when:** A simple static form may be acceptable with template-driven forms.
...
FormRecord supports dynamic string keys. Template-driven forms create models through directives; reactive forms create the tree explicitly in TypeScript.
...
- Putting raw values into FormArray instead of controls
- untyped nullable forms
- subscribing at every level
- rebuilding form
...
A payment screen uses **What are FormControl, FormGroup, and FormArray**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** FormControl represents one value/status, FormGroup represents named controls, and FormArray represents an ordered collection of controls. Together they form a reactive form tree.
- **Remember:** Control = field, Group = object, Array = repeated list.
