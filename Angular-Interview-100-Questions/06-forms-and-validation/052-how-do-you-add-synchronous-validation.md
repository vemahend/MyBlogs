...
A synchronous validator immediately returns validation errors or null based only on the current control/group value. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Validator asks “is this value locally acceptable now?”**

## 1. What problem does it solve?

A synchronous validator immediately returns validation errors or null based only on the current control/group value.
...
Validator asks “is this value locally acceptable now?” Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Angular composes validator functions and recalculates status when values change. Group validators can compare controls
2. typed errors should describe rule keys, not presentation text.
...
Require positive amount, supported currency, and different source/destination accounts before enabling transfer. The API repeats all financial validation.
...
```ts
const different:ValidatorFn=g=>g.value.from===g.value.to?{sameAccount:true}:null;
form=this.fb.nonNullable.group({from:'',to:'',amount:[0,Validators.min(.01)]},{validators:[different]});
```
...
**Use it when:** For deterministic local and cross-field rules.

**Avoid or reconsider it when:** For remote data or side effects; validators must remain pure.
...
Built-ins cover required/min/max/pattern. Custom control validators inspect one value; group validators express cross-field rules; async validators wait for external answers.
...
- Regex as business truth
- returning `{}` instead of null
- validator mutating controls
- error message inside validator
- forgetting cross-field update.
...
- **Definition:** A synchronous validator immediately returns validation errors or null based only on the current control/group value.
- **Remember:** Validator asks “is this value locally acceptable now?”
