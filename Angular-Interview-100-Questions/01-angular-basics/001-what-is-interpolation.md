...
Interpolation is one-way text binding from a component expression into a template text node. The syntax is double curly braces, for example `{{ customerName }}`. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Curly braces mean: calculate, stringify, display. Data travels component → view, never back.**

## 1. What problem does it solve?

Interpolation is one-way text binding from a component expression into a template text node. The syntax is double curly braces, for example `{{ customerName }}`.
...
Curly braces mean: calculate, stringify, display. Data travels component → view, never back. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Angular compiles the expression into a binding instruction. During synchronization/change detection it reevaluates the expression, compares it with the previous value, converts the result to text, and updates the DOM only when needed. Interpolation is escaped, so ordinary values are not interpreted as HTML.
...
A transfer confirmation can display `Amount: {{ amount | currency:"NZD" }}`. The API still owns the true amount; interpolation only renders the value.
...
```ts
@Component({template: `<p>Available: {{ balance | currency:'NZD' }}</p>`})
export class Balance { balance = 1250.50; }
```
...
**Use it when:** For small, side-effect-free text expressions and pure formatting pipes.

**Avoid or reconsider it when:** For event handling, two-way editing, complex calculations, or trusted-HTML decisions.
...
Interpolation writes text. Property binding writes a DOM/component property. Attribute binding writes an HTML attribute. Use `[textContent]="value"` when you specifically need the property form.
...
- Calling methods with expensive work from interpolation
- using interpolation for `innerHTML`
- assuming null values are formatted correctly
...
- **Definition:** Interpolation is one-way text binding from a component expression into a template text node. The syntax is double curly braces, for example `{{ customerName }}`.
- **Remember:** Curly braces mean: calculate, stringify, display. Data travels component → view, never back.
