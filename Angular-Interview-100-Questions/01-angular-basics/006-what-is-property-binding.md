...
Property binding sets a DOM element, directive, or component property from an Angular expression using `[property]="expression"`. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Square brackets are an inbox: a value goes into a target property.**

## 1. What problem does it solve?

Property binding sets a DOM element, directive, or component property from an Angular expression using `[property]="expression"`.
...
Square brackets are an inbox: a value goes into a target property. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Angular evaluates the expression and assigns the result without stringifying it. This preserves booleans, numbers, objects, and functions. Component input binding participates in the child’s input update lifecycle.
...
Bind `[disabled]="form.invalid || saving()"` so a transfer cannot be submitted while invalid or in flight. The backend must still enforce the rule.
...
```ts
<button [disabled]="form.invalid || saving()" [attr.aria-busy]="saving()">Pay</button>
```
...
**Use it when:** For booleans, objects, component inputs, styles/classes, and DOM properties.

**Avoid or reconsider it when:** For reacting to user actions; use event binding. Do not use property binding as a security control.
...
Interpolation usually produces text. Property binding preserves the value type. Attribute binding `[attr.aria-label]` writes an attribute when no suitable property exists.
...
- Using `disabled="false"` and still disabling the control
- binding unsafe HTML
- mutating object inputs
- confusing `attr.colspan` with a property.
...
- **Definition:** Property binding sets a DOM element, directive, or component property from an Angular expression using `[property]="expression"`.
- **Remember:** Square brackets are an inbox: a value goes into a target property.
