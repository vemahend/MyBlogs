...
A template is the declarative HTML blueprint Angular uses to render a component and connect it to state and actions. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **The template describes what the UI should look like for the current state; the component supplies that state.**

## 1. What problem does it solve?

A template is the declarative HTML blueprint Angular uses to render a component and connect it to state and actions.
...
The template describes what the UI should look like for the current state; the component supplies that state. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Angular parses template syntax at build time, type-checks it against the component, and compiles it into instructions for element creation, bindings, listeners, control flow, and child components. It is not concatenated HTML.
...
A payment template can show loading, an empty beneficiary list, a form, validation errors, and success state without manually manipulating the DOM.
...
```ts
@if (loading()) { <app-spinner /> } @else if (error()) { <app-error /> } @else { <app-payment [value]="payment()!" /> }
```
...
**Use it when:** For rendering, binding, control flow, and composing components.

**Avoid or reconsider it when:** For complex transformation, business decisions, API orchestration, or reusable domain logic.
...
Inline and external templates behave the same. `ng-template` is an unrendered fragment. `ng-container` groups without adding a DOM element. `@defer` creates a deferred dependency boundary.
...
- **Definition:** A template is the declarative HTML blueprint Angular uses to render a component and connect it to state and actions.
- **Remember:** The template describes what the UI should look like for the current state; the component supplies that state.
