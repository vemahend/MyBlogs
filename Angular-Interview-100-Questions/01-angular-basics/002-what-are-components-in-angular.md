...
A component is Angular’s main UI building block: a TypeScript class plus metadata, a template, styles, inputs, outputs, and an injector boundary. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Component = class + view + contract + lifecycle. If it has no view, it probably is not a component.**

## 1. What problem does it solve?

A component is Angular’s main UI building block: a TypeScript class plus metadata, a template, styles, inputs, outputs, and an injector boundary.
...
Component = class + view + contract + lifecycle. If it has no view, it probably is not a component. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. The compiler reads component metadata and produces view instructions. At runtime Angular creates the class, resolves injected dependencies, creates its view, binds inputs, runs lifecycle work, and destroys it with the view. Modern Angular favors standalone components.
...
Split a payment page into container, beneficiary picker, amount editor, summary, and result components. Each has a narrow contract and can be tested separately.
...
```ts
@Component({selector:'app-payment-summary', standalone:true, templateUrl:'./summary.html'})
export class PaymentSummary { payment = input.required<PaymentVm>(); }
```
...
**Use it when:** For visible, cohesive UI behavior with an explicit public contract.

**Avoid or reconsider it when:** For general domain rules, cross-feature state, HTTP integration, or simple DOM behavior without a view.
...
A component owns a region of UI. A directive adds behavior to an existing element. A service contains reusable non-visual logic or state. A pipe transforms display values.
...
- component providers accidentally creating multiple service instances.
...
- **Definition:** A component is Angular’s main UI building block: a TypeScript class plus metadata, a template, styles, inputs, outputs, and an injector boundary.
- **Remember:** Component = class + view + contract + lifecycle. If it has no view, it probably is not a component.
