...
Directives are classes that add behavior to elements or change view structure. Components are directives with their own templates. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Directive = behavior attached to markup. Component = markup plus behavior.**

## 1. What problem does it solve?

Directives are classes that add behavior to elements or change view structure. Components are directives with their own templates.
...
Directive = behavior attached to markup. Component = markup plus behavior. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Angular matches directive selectors while creating a view, instantiates them through DI, binds inputs/outputs, applies host behavior, and shares the element’s lifecycle.
...
A currency-input directive normalizes input and accessibility; a permission directive controls whether an approval template exists.
...
export class CurrencyInputDirective { /* host input/blur behavior */ }
```
...
**Use it when:** For reusable, host-related behavior without its own view.

**Avoid or reconsider it when:** For complex UI that deserves a component or non-visual behavior that belongs in a service.
...
Attribute directives change behavior/appearance; structural directives control embedded views; components introduce a new view boundary.
...
- DOM manipulation bypassing renderer/platform concerns
- hidden side effects
- using directives for business workflows.
...
- **Definition:** Directives are classes that add behavior to elements or change view structure. Components are directives with their own templates.
- **Remember:** Directive = behavior attached to markup. Component = markup plus behavior.
