...
`ngIf` conditionally creates a view and `ngFor` repeats a view for a collection. Modern built-in control flow uses `@if` and `@for`. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Conditional rendering changes existence; CSS changes visibility.**

## 1. What problem does it solve?

`ngIf` conditionally creates a view and `ngFor` repeats a view for a collection. Modern built-in control flow uses `@if` and `@for`.
...
Conditional rendering changes existence; CSS changes visibility. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. False conditions remove/destroy the embedded view rather than merely hiding CSS. Repetition creates a view per item and uses identity tracking to reuse the correct DOM when data changes.
...
Show a transfer approval panel only when eligible; render transaction rows with `@for (tx of transactions(); track tx.id)`.
...
```ts
@for (tx of transactions(); track tx.id) { <app-transaction-row [tx]="tx" /> } @empty { <p>No transactions</p> }
```
...
**Use it when:** For conditional or repeated UI with clear view identity.

**Avoid or reconsider it when:** Use CSS visibility when state/DOM must remain; use virtual scrolling for huge collections.
...
CSS hiding keeps the element alive. Conditional control flow removes it. `@for` track expressions replace the older `trackBy` function style in modern templates.
...
- Tracking by index for reordered data
- functions in conditions
- deeply nested blocks
- expecting hidden components to preserve state after destruction.
...
- **Definition:** `ngIf` conditionally creates a view and `ngFor` repeats a view for a collection. Modern built-in control flow uses `@if` and `@for`.
- **Remember:** Conditional rendering changes existence; CSS changes visibility.
