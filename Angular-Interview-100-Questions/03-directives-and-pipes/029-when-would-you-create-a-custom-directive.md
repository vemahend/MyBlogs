...
Create a custom directive when multiple elements need the same DOM behavior, interaction, accessibility rule, or conditional view policy. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Shared behavior + caller-owned markup = directive.**

## 1. What problem does it solve?

Create a custom directive when multiple elements need the same DOM behavior, interaction, accessibility rule, or conditional view policy.
...
Shared behavior + caller-owned markup = directive. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. The directive can inject its host, renderer, template/view container, services, and destroy context. It exposes typed inputs/outputs and follows host lifecycle.
...
A `preventDoubleSubmit` directive can disable a payment button while its command is pending—but idempotency remains required at the API.
...
```ts
@Directive({selector:'button[appPreventDoubleSubmit]'})
export class PreventDoubleSubmit { pending = input(false); }
```
...
**Use it when:** When behavior repeats and has a clean host contract.

**Avoid or reconsider it when:** When CSS, a utility function, component, or built-in directive is sufficient.
...
Use a component when reusable markup is central; a directive when the host markup remains owned by the caller; a service for non-DOM logic.
...
- Building a generic directive for one use
- hiding network calls
- failing SSR/platform checks
- conflicting with forms
- no keyboard/accessibility tests.
...
A payment screen uses **When would you create a custom directive**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Create a custom directive when multiple elements need the same DOM behavior, interaction, accessibility rule, or conditional view policy.
- **Remember:** Shared behavior + caller-owned markup = directive.
