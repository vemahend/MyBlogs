...
A directive packages reusable DOM or element behavior so templates do not repeat the same bindings, listeners, permission checks, or accessibility behavior. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **If the repeated thing is element behavior, think directive—not copy/paste.**

## 1. What problem does it solve?

A directive packages reusable DOM or element behavior so templates do not repeat the same bindings, listeners, permission checks, or accessibility behavior.
...
If the repeated thing is element behavior, think directive—not copy/paste. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Attribute directives attach behavior to an existing host
2. structural directives create/remove embedded views. Host bindings/listeners and injected services let the directive apply policy consistently.
...
A `hasPermission` structural directive removes unauthorized payment actions, while the API still enforces authorization. A focus-invalid directive focuses the first invalid control after submit.
...
```ts
@Directive({selector:'[appFocusInvalid]'})
export class FocusInvalidDirective { /* focus first invalid control after submit */ }
```
...
**Use it when:** When multiple templates repeat the same element/view behavior.

**Avoid or reconsider it when:** When a component, CSS class, pipe, or plain function is simpler.
...
A component owns a view; a directive decorates/controls another view; a pipe transforms display data; a function handles non-DOM logic.
...
- Putting business authorization only in a directive
- directives with hidden global state
- too many magic behaviors
- competing host bindings.
...
A payment screen uses **How do directives help reduce duplicated template logic**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** A directive packages reusable DOM or element behavior so templates do not repeat the same bindings, listeners, permission checks, or accessibility behavior.
- **Remember:** If the repeated thing is element behavior, think directive—not copy/paste.
