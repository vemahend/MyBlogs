...
Create a custom pipe for a repeated, deterministic, presentation-only transformation that reads naturally in a template. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **If the output is just another way to display the same value, a pipe may fit.**

## 1. What problem does it solve?

Create a custom pipe for a repeated, deterministic, presentation-only transformation that reads naturally in a template.
...
If the output is just another way to display the same value, a pipe may fit. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. A pure pipe instance transforms input and arguments and reruns only when relevant identity/value changes. It should not mutate input or depend on invisible global state.
...
`{{ accountNumber | maskAccount }}` consistently displays only the final four digits. The raw value should already be access-controlled and should not be logged.
...
```ts
@Pipe({name:'paymentLabel', standalone:true, pure:true})
export class PaymentLabelPipe { transform(s:PaymentStatus){ return LABELS[s]; } }
```
...
**Use it when:** For small, pure, reused display transformations.

**Avoid or reconsider it when:** For one-off simple expressions, domain calculation, validation, or operations with side effects.
...
A pipe formats for display; a DTO mapper changes architecture-boundary shape; a validator decides validity; a computed signal derives reactive state.
...
- Async API calls
- impure filtering
- localization reinvented instead of built-ins
- using masking as data protection
- swallowing invalid input.
...
A payment screen uses **When would you create a custom pipe**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Create a custom pipe for a repeated, deterministic, presentation-only transformation that reads naturally in a template.
- **Remember:** If the output is just another way to display the same value, a pipe may fit.
