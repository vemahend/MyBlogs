...
Maintainable templates keep expressions simple, state explicit, branches shallow, and repeated UI extracted behind meaningful components/directives/pipes. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **If the template reads like a program, move the program out.**

## 1. What problem does it solve?

Maintainable templates keep expressions simple, state explicit, branches shallow, and repeated UI extracted behind meaningful components/directives/pipes.
...
If the template reads like a program, move the program out. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Every template expression participates in rendering. Complex calls can rerun frequently and obscure dependencies. Deep conditional structure also creates difficult lifecycle and accessibility behavior.
...
Create a `PaymentStatusViewModel` in TypeScript, then let the template choose clear loading/error/empty/content states instead of calling several permission and formatting methods per row.
...
readonly vm = computed(() => ({
  showApproval: this.payment()?.status === 'pending' && this.canApprove(),
  formattedReference: this.payment()?.reference?.trim() ?? '—'
}));
```
...
**Use it when:** Keep simple bindings and declarative control flow in templates.

**Avoid or reconsider it when:** Do not extract every one-line condition; optimize for clarity, not maximum file count.
...
A smart/container component prepares state; presentational templates render it. A pipe handles display transformation; a method/service handles behavior.
...
- Nested subscriptions via template tricks
- method calls per row
- giant ternaries
- duplicated permissions
- excessive `async`
- no track identity
- inaccessible click-only elements.
...
A payment screen uses **What mistakes can make templates hard to maintain**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Maintainable templates keep expressions simple, state explicit, branches shallow, and repeated UI extracted behind meaningful components/directives/pipes.
- **Remember:** If the template reads like a program, move the program out.
