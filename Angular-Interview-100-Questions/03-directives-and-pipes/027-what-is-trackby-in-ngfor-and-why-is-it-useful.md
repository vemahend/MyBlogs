...
`trackBy` tells `*ngFor` how to identify an item so Angular can reuse the correct DOM/component instance when the collection reference or order changes. Modern `@for` uses `track item.id`. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Track the business identity, not the row position.**

## 1. What problem does it solve?

`trackBy` tells `*ngFor` how to identify an item so Angular can reuse the correct DOM/component instance when the collection reference or order changes. Modern `@for` uses `track item.id`.
...
Track the business identity, not the row position. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Angular compares previous and current item identities. Stable keys let it move/reuse views rather than destroy/recreate them, preserving focus and child state and reducing DOM work.
...
Track transaction rows by immutable transaction ID. A refreshed API array then updates rows without recreating every action menu and formatter.
...
// Modern: @for (tx of transactions; track tx.id) { ... }
```
...
**Use it when:** For dynamic/reloaded/reordered lists with stable unique IDs.

**Avoid or reconsider it when:** Not necessary for tiny immutable lists, but explicit identity is still good documentation.
...
Default identity often uses object reference. Tracking by index is safe only for genuinely append-only/static ordering. Virtual scrolling solves a different problem: not rendering off-screen rows.
...
- Non-unique keys
- changing keys
- index tracking during sorting
- returning a new random value
- believing tracking reduces network or JavaScript mapping cost.
...
A payment screen uses **What is trackBy in ngFor, and why is it useful**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** `trackBy` tells `*ngFor` how to identify an item so Angular can reuse the correct DOM/component instance when the collection reference or order changes. Modern `@for` uses `track item.id`.
- **Remember:** Track the business identity, not the row position.
