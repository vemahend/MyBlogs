...
Structural directives control whether/how template fragments exist; attribute directives change behavior or appearance of existing elements. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Structural changes the skeleton; attribute changes the behavior/clothing.**

## 1. What problem does it solve?

Structural directives control whether/how template fragments exist; attribute directives change behavior or appearance of existing elements.
...
Structural changes the skeleton; attribute changes the behavior/clothing. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. A structural directive receives a `TemplateRef` and manages embedded views in a `ViewContainerRef`. Attribute directives operate on a host already being rendered.
...
`hasPermission` may conditionally create an Approve button; `currencyInput` may format and validate an existing input.
...
```ts
// Structural policy controls existence; attribute behavior modifies a host.
@if (canApprove()) { <button appPreventDoubleClick>Approve</button> }
```
...
**Use it when:** Structural for reusable view creation policy; attribute for reusable host behavior.

**Avoid or reconsider it when:** Use built-in control flow first; create custom structural directives only when a repeated policy is truly domain-wide.
...
Modern `@if/@for` are built-in template control-flow blocks, not custom star-syntax directives, but solve similar structural rendering needs.
...
- Multiple star structural directives on one element
- using structural UI as server authorization
- attribute directive replacing full component responsibilities.
...
A payment screen uses **What is the difference between structural and attribute directives**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Structural directives control whether/how template fragments exist; attribute directives change behavior or appearance of existing elements.
- **Remember:** Structural changes the skeleton; attribute changes the behavior/clothing.
