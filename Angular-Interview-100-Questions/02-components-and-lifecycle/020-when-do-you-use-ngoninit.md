...
`ngOnInit` performs one-time initialization after Angular has set the component’s initial inputs. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **OnInit means once per instance, not once per application or once per URL forever.**

## 1. What problem does it solve?

`ngOnInit` performs one-time initialization after Angular has set the component’s initial inputs.
...
OnInit means once per instance, not once per application or once per URL forever. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. It runs once per component instance, after the first input-change processing and before the view is fully initialized. Navigation can create a new instance
2. reused instances may require reacting to parameter/input streams instead.
...
Initialize a payment screen’s view model and start a route-driven load. If route parameters can change without recreating the component, subscribe/react to the parameter stream rather than relying on OnInit once.
...
```ts
ngOnInit(){
  this.route.paramMap.pipe(map(p=>p.get('id')!), switchMap(id=>this.api.get(id)), takeUntilDestroyed()).subscribe();
}
```
...
**Use it when:** For one-time instance setup that needs Angular-bound values.

**Avoid or reconsider it when:** For repeated reactions, cleanup, or DOM measurements.
...
Use computed signals for derived state, input changes for repeated input reactions, and after-view work for view-query-dependent integration.
...
- Assuming it reruns on parameter change
- nested subscriptions
- no loading/error state
- multiple initial API calls
...
- **Definition:** `ngOnInit` performs one-time initialization after Angular has set the component’s initial inputs.
- **Remember:** OnInit means once per instance, not once per application or once per URL forever.
