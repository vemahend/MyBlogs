...
The constructor creates the class and receives injected dependencies; `ngOnInit` runs after Angular has assigned initial inputs. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.
...
The constructor creates the class and receives injected dependencies; `ngOnInit` runs after Angular has assigned initial inputs.
...
Constructor: “what I need.” OnInit: “what I do once Angular has set me up.” Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. TypeScript/JavaScript calls the constructor first. Angular then binds inputs and invokes initialization hooks. Heavy side effects in a constructor make creation surprising and hard to test.
...
Inject `PaymentApi` in construction, but load a payment in `ngOnInit` after the required `paymentId` input or route context is available.
...
ngOnInit(){ this.api.get(this.paymentId).subscribe(); }
```
...
**Use it when:** Use constructor for DI/simple initialization and OnInit for one-time Angular-dependent setup.

**Avoid or reconsider it when:** Do not use OnInit when a computed value or input transform expresses the behavior declaratively.
...
Field initializers and constructor establish instance invariants; `ngOnInit` performs Angular-aware initialization; input-driven recalculation belongs in `ngOnChanges` or computed signals.
...
- duplicated load in constructor and hook
- hiding async initialization failures.
...
A payment screen uses **What is the difference between constructor and ngOnInit**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** The constructor creates the class and receives injected dependencies; `ngOnInit` runs after Angular has assigned initial inputs.
- **Remember:** Constructor: “what I need.” OnInit: “what I do once Angular has set me up.”
