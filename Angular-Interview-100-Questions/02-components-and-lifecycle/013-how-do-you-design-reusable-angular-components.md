...
Reusable components solve one cohesive UI problem through a small, typed, stable API rather than knowing a particular screen or backend. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Reusable means reusable by contract, not configurable with fifty flags.**

## 1. What problem does it solve?

Reusable components solve one cohesive UI problem through a small, typed, stable API rather than knowing a particular screen or backend.
...
Reusable means reusable by contract, not configurable with fifty flags. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Inputs configure data/behavior, outputs express semantic events, content projection creates slots, and dependency injection can supply cross-cutting policy. Accessibility and styling are part of the contract.
...
A reusable money input accepts currency, min/max, and disabled state; emits a parsed decimal value; supports keyboard/screen reader usage; and never calls the payment API itself.
...
**Use it when:** When at least two contexts share the same behavior and semantics.

**Avoid or reconsider it when:** Do not generalize components whose similarity is only visual or likely to diverge.
...
Presentational components are reusable and stateless-ish. Container components integrate state/services. ControlValueAccessor integrates a custom field with Angular forms.
...
- passing raw DTOs
- emitting DOM events rather than intent
...
- **Definition:** Reusable components solve one cohesive UI problem through a small, typed, stable API rather than knowing a particular screen or backend.
- **Remember:** Reusable means reusable by contract, not configurable with fifty flags.
