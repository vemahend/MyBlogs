...
Parent-child communication should use explicit contracts: inputs pass data down, outputs send user intent up, content projection supplies markup, and view/content queries are reserved for imperative integration. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Data down, events up. The child reports what happened; the parent decides what it means.**

## 1. What problem does it solve?

Parent-child communication should use explicit contracts: inputs pass data down, outputs send user intent up, content projection supplies markup, and view/content queries are reserved for imperative integration.
...
Data down, events up. The child reports what happened; the parent decides what it means. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Angular binds parent expressions to child inputs. When the reference/value changes, the child is updated. Outputs notify subscribed parents without knowing them, preserving direction and testability.
...
A parent transfer page passes available accounts into an account picker; the picker emits `accountSelected`; the parent updates the form and reloads limits.
...
```ts
<app-account-picker [accounts]="accounts()" (accountSelected)="selectAccount($event)" />
```
...
**Use it when:** For direct, reusable parent-child relationships.

**Avoid or reconsider it when:** Use a service/store when communication skips levels or represents shared application state.
...
Inputs/outputs suit direct hierarchy. A shared service or store suits unrelated components. Router state suits shareable navigation state.
...
- Child injecting its parent
- mutating input arrays
- output named `click`
- too many round trips
- using a global service for a local relationship.
...
A payment screen uses **How do parent and child components communicate**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Parent-child communication should use explicit contracts: inputs pass data down, outputs send user intent up, content projection supplies markup, and view/content queries are reserved for imperative integration.
- **Remember:** Data down, events up. The child reports what happened; the parent decides what it means.
