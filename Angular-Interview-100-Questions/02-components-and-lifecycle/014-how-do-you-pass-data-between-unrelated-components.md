...
Unrelated components communicate through an appropriate shared boundary: a scoped service/store, router URL state, or backend persistence. Avoid reaching through the DOM or component tree. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.
...
Unrelated components communicate through an appropriate shared boundary: a scoped service/store, router URL state, or backend persistence. Avoid reaching through the DOM or component tree.
...
Choose the lifetime first; then choose the communication mechanism. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. A service instance can expose readonly signals/observables and command methods. Provider placement determines whether state is application-wide, route-scoped, or component-subtree-scoped.
...
A dashboard account selector and a transfer form share `SelectedAccountStore`, provided at the payments route so state disappears when the feature is left.
...
```ts
@Injectable() export class SelectedAccountStore {
  private selected = signal<Account|null>(null);
  readonly value = this.selected.asReadonly();
  select(a:Account){this.selected.set(a)}
}
```
...
**Use it when:** For state that genuinely has multiple consumers or must outlive one component.

**Avoid or reconsider it when:** Do not replace clear parent-child contracts with a global event bus.
...
URL state is best for shareable filters; a service/store is best for in-memory workflow state; backend persistence is best across sessions/devices.
...
- Global singleton for every state
- public writable subjects
- event-bus spaghetti
- lost refresh state
- storing sensitive form data in localStorage.
...
- **Definition:** Unrelated components communicate through an appropriate shared boundary: a scoped service/store, router URL state, or backend persistence. Avoid reaching through the DOM or component tree.
- **Remember:** Choose the lifetime first; then choose the communication mechanism.
