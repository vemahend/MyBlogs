...
A component should coordinate the view, not own every concern. Move API access to clients, business decisions to domain/facade services, transformations to pure functions, and reusable view behavior to directives/pipes. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Component = conductor, not the entire orchestra.**

## 1. What problem does it solve?

A component should coordinate the view, not own every concern. Move API access to clients, business decisions to domain/facade services, transformations to pure functions, and reusable view behavior to directives/pipes.
...
Component = conductor, not the entire orchestra. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Thin components bind view state and delegate work. This reduces dependencies and makes business behavior testable without rendering. A facade can combine several services and expose a small screen-oriented API.
...
The transfer component collects input and displays state; a `PaymentFacade` validates workflow, calls the API, and maps errors; the server enforces financial rules.
...
export class TransferPage {
  ...
  submit(v: PaymentDraft){ this.facade.submit(v); }
}
```
...
**Use it when:** Extract when logic is reusable, independently testable, stateful beyond the view, or integration-related.

**Avoid or reconsider it when:** Keep tiny display decisions close to the template; avoid architecture ceremony for trivial components.
...
A facade is screen-oriented orchestration; a domain service is reusable business behavior; a store owns shared state; a component owns presentation.
...
- Moving all logic to one giant service
- leaking subscriptions
- service methods returning `any`
- components directly mapping raw DTOs
- hiding simple UI logic behind unnecessary layers.
...
A payment screen uses **How do you avoid putting too much logic in a component**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** A component should coordinate the view, not own every concern. Move API access to clients, business decisions to domain/facade services, transformations to pure functions, and reusable view behavior to directives/pipes.
- **Remember:** Component = conductor, not the entire orchestra.
