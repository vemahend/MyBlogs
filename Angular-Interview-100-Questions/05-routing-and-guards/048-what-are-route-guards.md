...
Route guards are navigation policy functions that allow, cancel, redirect, or confirm entering/leaving routes. They improve workflow and UX but are not server authorization. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Guards control navigation, not data access security.**

## 1. What problem does it solve?

Route guards are navigation policy functions that allow, cancel, redirect, or confirm entering/leaving routes. They improve workflow and UX but are not server authorization.
...
Guards control navigation, not data access security. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. The router executes relevant guard types during navigation and uses their first completed result. Returning a UrlTree/redirect is preferred to starting a second navigation imperatively.
...
An auth guard checks session, a permission guard checks approver capability, and a deactivate guard warns about an unsaved transfer draft.
...
```ts
export const permissionGuard:CanActivateFn=()=>inject(Policy).canApprove()?true:inject(Router).createUrlTree(['/forbidden']);
```
...
**Use it when:** For navigation prerequisites and unsaved-change confirmation.

**Avoid or reconsider it when:** For server-only rules, routine data loading, or hiding all error handling.
...
CanActivate enters; CanDeactivate leaves; CanMatch controls route matching/loading; resolver fetches data; API authorization enforces security.
...
- Guard Observable never completes
- side effects
- duplicate auth checks
- relying on client roles
- no refresh initialization
- generic false leaves blank screen.
...
- **Definition:** Route guards are navigation policy functions that allow, cancel, redirect, or confirm entering/leaving routes. They improve workflow and UX but are not server authorization.
- **Remember:** Guards control navigation, not data access security.
