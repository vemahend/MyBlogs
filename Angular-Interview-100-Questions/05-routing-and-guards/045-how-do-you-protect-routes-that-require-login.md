...
Protect login-required routes with a guard that checks/restores authentication state and returns a redirect URL tree when access is unavailable. The API must independently authorize every request. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Guard the door for UX; the API guards the vault.**

## 1. What problem does it solve?

Protect login-required routes with a guard that checks/restores authentication state and returns a redirect URL tree when access is unavailable. The API must independently authorize every request.
...
Guard the door for UX; the API guards the vault. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. A guard may return boolean, UrlTree/redirect command, Promise, or Observable. Navigation waits for the result. On refresh, auth restoration may be asynchronous, so distinguish unknown/loading from unauthenticated.
...
An approval route waits until the session is restored, checks the approval permission, and redirects to login or forbidden while retaining a safe return URL.
...
```ts
export const authGuard:CanActivateFn=()=>inject(SessionStore).ready$.pipe(map(s=>s.authenticated||inject(Router).createUrlTree(['/login'])));
```
...
**Use it when:** For authentication, authorization hints, prerequisites, or feature access.

**Avoid or reconsider it when:** Do not put slow business data fetching in guards when resolver/page loading is clearer.
...
Guard controls navigation UX; menu visibility controls discoverability; API authorization controls security. `CanMatch` can select route definitions before loading.
...
- Reading stale in-memory auth on refresh
- imperative `navigate` plus false
- guard never completes
- role names duplicated
- redirect loop
- trusting guard alone.
...
A payment screen uses **How do you protect routes that require login**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Protect login-required routes with a guard that checks/restores authentication state and returns a redirect URL tree when access is unavailable. The API must independently authorize every request.
- **Remember:** Guard the door for UX; the API guards the vault.
