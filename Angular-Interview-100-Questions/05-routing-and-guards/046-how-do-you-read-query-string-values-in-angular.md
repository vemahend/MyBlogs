...
Read query strings with `ActivatedRoute.queryParamMap` or input binding, parse them into typed values, and react when they change. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **URL values are untrusted strings, not typed application state.**

## 1. What problem does it solve?

Read query strings with `ActivatedRoute.queryParamMap` or input binding, parse them into typed values, and react when they change.
...
URL values are untrusted strings, not typed application state. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. The observable emits on navigation even if the component instance is reused. Snapshot reads once and can become stale. Values are strings and require validation/defaults.
...
Parse transaction `status`, `page`, and `fromDate`; reject unknown values and cap page size before requesting the API.
...
```ts
readonly filters$=this.route.queryParamMap.pipe(map(p=>({status:parseStatus(p.get('status')),page:Math.max(1,Number(p.get('page')??1))})));
```
...
**Use it when:** For reactive filters, sort, pagination, tabs, and campaign/referral data.

**Avoid or reconsider it when:** Do not use query parameters as hidden secure state.
...
Snapshot is simple one-time state; observable handles reuse; signals can be created from the stream; router input binding can map route values directly.
...
- `Number(null)` surprises
- no unsubscribe/interop
- snapshot while params change
- duplicate request from multiple subscriptions
- sensitive filters in URL.
...
- **Definition:** Read query strings with `ActivatedRoute.queryParamMap` or input binding, parse them into typed values, and react when they change.
- **Remember:** URL values are untrusted strings, not typed application state.
