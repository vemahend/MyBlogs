...
Child routes model nested navigation under a parent route and render into that parent component’s `router-outlet`. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Parent route supplies the frame; child route supplies the page inside it.**

## 1. What problem does it solve?

Child routes model nested navigation under a parent route and render into that parent component’s `router-outlet`.
...
Parent route supplies the frame; child route supplies the page inside it. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. The router first matches the parent path, then matches remaining URL segments against `children`. Parent guards/providers/resolvers can form a feature boundary
2. relative navigation starts from the active route.
...
`/payments/:id/details`, `/history`, and `/approve` share a payment shell, header, scoped store, and authorization policy.
...
```ts
{path:'payments/:id',component:PaymentShell,children:[{path:'details',component:PaymentDetails},{path:'history',component:PaymentHistory}]}
```
...
**Use it when:** For a feature shell with nested pages, shared context, or step navigation.

**Avoid or reconsider it when:** For purely visual child components that do not deserve URLs.
...
Child routes express URL hierarchy. Child components express view hierarchy; the two often align but do not have to.
...
- Missing child outlet
- absolute navigation breaking context
- duplicated providers in children
- parent parameter ignored
- overly deep route trees.
...
A payment screen uses **How do you configure child routes**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Child routes model nested navigation under a parent route and render into that parent component’s `router-outlet`.
- **Remember:** Parent route supplies the frame; child route supplies the page inside it.
