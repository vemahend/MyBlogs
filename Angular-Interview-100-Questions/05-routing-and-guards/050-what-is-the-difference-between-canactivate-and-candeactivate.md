...
`CanActivate` decides whether a route may be entered; `CanDeactivate` decides whether the current component may be left, commonly to protect unsaved work. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.
...
`CanActivate` decides whether a route may be entered; `CanDeactivate` decides whether the current component may be left, commonly to protect unsaved work.
...
Activate protects entry; deactivate protects exit. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. CanActivate receives the target route/state. CanDeactivate receives the component plus current/next navigation context and can synchronously or asynchronously confirm departure.
...
CanActivate checks payment-approver access. CanDeactivate prompts when a transfer draft is dirty, but allows leaving after successful save or explicit discard.
...
```ts
export const leaveDraftGuard:CanDeactivateFn<TransferPage>=c=>!c.form.dirty||c.saved()||window.confirm('Discard unsaved transfer?');
```
...
**Use it when:** Activate for prerequisites; deactivate for genuine unsaved/destructive exit risk.

**Avoid or reconsider it when:** Do not annoy users with prompts for recoverable or unchanged state.
...
CanMatch prevents route matching and may avoid lazy loading; CanActivate runs for an already matched route; CanDeactivate protects current view state.
...
- Browser confirm in unit tests
- prompting after successful save
- guard coupled to component internals
- API security delegated to CanActivate
- navigation loops.
...
- **Definition:** `CanActivate` decides whether a route may be entered; `CanDeactivate` decides whether the current component may be left, commonly to protect unsaved work.
- **Remember:** Activate protects entry; deactivate protects exit.
