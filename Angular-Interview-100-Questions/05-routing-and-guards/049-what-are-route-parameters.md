...
Route parameters are dynamic path segments such as `/payments/:paymentId` that identify a resource or hierarchical context. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Path answers “which thing?” Query answers “how should I view it?”**

## 1. What problem does it solve?

Route parameters are dynamic path segments such as `/payments/:paymentId` that identify a resource or hierarchical context.
...
Path answers “which thing?” Query answers “how should I view it?” Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. The router matches the segment and exposes it through `paramMap`. A reused component can receive new parameters without reconstruction, so reactive handling is safer than assuming OnInit will rerun.
...
`/accounts/:accountId/payments/:paymentId` identifies a payment in account context. The API still verifies the signed-in user can access both.
...
```ts
readonly payment$=this.route.paramMap.pipe(map(p=>p.get('paymentId')!),distinctUntilChanged(),switchMap(id=>this.api.get(id)));
```
...
**Use it when:** For durable resource identity and meaningful hierarchy.

**Avoid or reconsider it when:** For optional filters, secrets, large payloads, or temporary state.
...
Path params identify the route/resource; query params modify view options; route data is static configuration; state services hold non-URL workflow data.
...
- Snapshot stale on same component
- unencoded IDs
- sequential IDs exposed without authorization
- parameter name mismatch
- nested parameter lookup confusion.
...
- **Definition:** Route parameters are dynamic path segments such as `/payments/:paymentId` that identify a resource or hierarchical context.
- **Remember:** Path answers “which thing?” Query answers “how should I view it?”
