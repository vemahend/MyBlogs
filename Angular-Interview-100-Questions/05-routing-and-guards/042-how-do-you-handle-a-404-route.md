...
A 404 route is a final wildcard route that displays a not-found page, preserves useful diagnostics, and avoids silently redirecting every unknown URL to home. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Three 404s: route, resource, hosting. Diagnose the right layer.**

## 1. What problem does it solve?

A 404 route is a final wildcard route that displays a not-found page, preserves useful diagnostics, and avoids silently redirecting every unknown URL to home.
...
Three 404s: route, resource, hosting. Diagnose the right layer. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Route configuration is evaluated in order
2. `**` matches any remaining URL, so it must come last. The web server must also rewrite valid SPA routes to the app entry while serving real static 404s correctly.
...
An invalid `/payments/XYZ` may be a valid route with a missing resource (API 404), while `/paymnts` is an unknown client route. Give distinct messages and actions.
...
```ts
export const routes:Routes=[{path:'payments/:id',component:PaymentPage},{path:'**',loadComponent:()=>import('./not-found').then(m=>m.NotFoundPage)}];
```
...
**Use it when:** Always provide a last wildcard and an accessible recovery page.

**Avoid or reconsider it when:** Do not convert every API error to the client 404 screen.
...
Router 404 means no route matched. API 404 means the resource does not exist or is intentionally undisclosed. Server 404 can mean SPA fallback is misconfigured.
...
- Wildcard first
- redirect loop
- losing requested URL
- production server not rewriting deep links
- exposing whether a restricted payment exists.
...
- **Definition:** A 404 route is a final wildcard route that displays a not-found page, preserves useful diagnostics, and avoids silently redirecting every unknown URL to home.
- **Remember:** Three 404s: route, resource, hosting. Diagnose the right layer.
