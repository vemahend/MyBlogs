...
An Angular service is an injectable class or provider that supplies non-visual behavior, integration, state, or policy to consumers. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **No template + reusable capability usually means service.**

## 1. What problem does it solve?

An Angular service is an injectable class or provider that supplies non-visual behavior, integration, state, or policy to consumers.
...
No template + reusable capability usually means service. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Services have no special view
2. DI controls creation and lifetime. They can expose synchronous methods, signals, or Observables and can themselves inject other capabilities.
...
`PaymentApi` calls endpoints, `PaymentFacade` orchestrates screen flow, and `PaymentDraftStore` owns feature state. Keeping these distinct avoids one god service.
...
```ts
@Injectable({providedIn:'root'}) export class PaymentApi { private http=inject(HttpClient); get(id:string){return this.http.get<PaymentDto>('/api/payments/'+id)} }
```
...
**Use it when:** For reusable logic, data access, orchestration, state, configuration, and adapters.

**Avoid or reconsider it when:** For tiny component-local display logic or markup behavior better expressed as a directive.
...
A component renders; a service performs non-visual work; a pipe transforms display; a guard controls navigation; an interceptor wraps HTTP transport.
...
- Stateful root singleton by accident
- returning mutable subjects
- swallowing errors
- mixing unrelated responsibilities
- service directly manipulating DOM.
...
- **Definition:** An Angular service is an injectable class or provider that supplies non-visual behavior, integration, state, or policy to consumers.
- **Remember:** No template + reusable capability usually means service.
