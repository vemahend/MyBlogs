...
Organize large-app services by business feature and responsibility, not one global `services` dumping ground. Separate API clients, facades, stores, domain utilities, and cross-cutting platform services. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Keep things close to the feature that changes with them.**

## 1. What problem does it solve?

Organize large-app services by business feature and responsibility, not one global `services` dumping ground. Separate API clients, facades, stores, domain utilities, and cross-cutting platform services.
...
Keep things close to the feature that changes with them. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Feature boundaries can lazy-load code and provide scoped dependencies. Public barrels expose only intended contracts. Dependency rules stop shared/core layers from importing features.
...
`payments/data-access/payment.api.ts`, `payments/application/payment.facade.ts`, `payments/state/payment.store.ts`; root platform has auth, telemetry, and configuration.
...
payments/
  ...
  ui/payment-summary.component.ts
...
**Use it when:** For enterprise apps with clear feature ownership and dependency rules.

**Avoid or reconsider it when:** Avoid excessive layers for a small app; structure should earn its complexity.
...
Core/platform is app-wide infrastructure; shared UI is domain-neutral; feature services belong with the feature. A facade is not automatically a repository or store.
...
- Huge shared folder
- one service per entity with mixed duties
- feature-to-feature imports
- circular barrels
- raw HttpClient in components.
...
- **Definition:** Organize large-app services by business feature and responsibility, not one global `services` dumping ground. Separate API clients, facades, stores, domain utilities, and cross-cutting platform services.
- **Remember:** Keep things close to the feature that changes with them.
