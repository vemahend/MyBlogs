...
Avoid circular service dependencies by fixing responsibility boundaries rather than using injector tricks. Cycles usually show that services know too much about each other. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Dependencies should form a directed graph, not a round trip.**

## 1. What problem does it solve?

Avoid circular service dependencies by fixing responsibility boundaries rather than using injector tricks. Cycles usually show that services know too much about each other.
...
Dependencies should form a directed graph, not a round trip. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. If A requires B while B requires A, construction cannot form a clean dependency graph. `forwardRef` can resolve declaration-order issues but does not repair an architectural cycle.
...
Split `PaymentService` and `NotificationService` coordination into `PaymentWorkflow`; communicate state through a store or domain event only when it truly represents a decoupled event.
...
```ts
@Injectable() export class PaymentWorkflow { constructor(private payments:PaymentService,private notices:NotificationService){} }
```
...
**Use it when:** Refactor into lower-level shared capability, coordinator, or inverted port.

**Avoid or reconsider it when:** Do not introduce events merely to avoid a simple direct call where no cycle exists.
...
A mediator/facade coordinates peers. An event breaks direct knowledge but adds asynchronous complexity. Extracting an interface/token can invert a dependency.
...
- Using `Injector.get()` to hide cycle
- mutual callbacks
- god services
- event bus to conceal synchronous coupling
...
A payment screen uses **How do you avoid circular dependencies between services**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Avoid circular service dependencies by fixing responsibility boundaries rather than using injector tricks. Cycles usually show that services know too much about each other.
- **Remember:** Dependencies should form a directed graph, not a round trip.
