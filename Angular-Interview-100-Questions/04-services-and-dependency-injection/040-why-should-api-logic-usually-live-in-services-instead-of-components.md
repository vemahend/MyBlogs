...
API logic belongs in services so components remain focused on view state and user orchestration, while endpoint construction, typing, mapping, and transport policy stay reusable and testable. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Components should not know URLs, headers, or raw DTO quirks.**

## 1. What problem does it solve?

API logic belongs in services so components remain focused on view state and user orchestration, while endpoint construction, typing, mapping, and transport policy stay reusable and testable.
...
Components should not know URLs, headers, or raw DTO quirks. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. A typed API client calls HttpClient and maps transport DTOs. A facade may combine calls and expose a view model. Interceptors handle cross-cutting headers/errors.
...
The transfer page calls `PaymentFacade.submit(draft)`; the facade maps the request, the API client posts it, and the component renders pending/success/failure states.
...
create(dto:CreatePaymentDto){return this.http.post<PaymentDto>('/api/payments',dto)}
```
...
**Use it when:** For all non-trivial backend integration and reuse across screens.

**Avoid or reconsider it when:** A tiny prototype may begin locally, but extract before duplication and mixed responsibilities grow.
...
API client owns HTTP details; facade owns screen workflow; domain mapper owns boundary conversion; component owns interaction/rendering.
...
- duplicated endpoints/error mapping
- nested subscriptions
- component tied to transport DTO
- impossible isolated tests.
...
- **Definition:** API logic belongs in services so components remain focused on view state and user orchestration, while endpoint construction, typing, mapping, and transport policy stay reusable and testable.
- **Remember:** Components should not know URLs, headers, or raw DTO quirks.
