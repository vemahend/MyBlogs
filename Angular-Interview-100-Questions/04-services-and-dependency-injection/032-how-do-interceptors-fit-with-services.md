...
HTTP interceptors are middleware around `HttpClient`. They handle cross-cutting transport concerns while API services express domain-specific endpoints. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Service says what endpoint; interceptor says what every HTTP trip must carry.**

## 1. What problem does it solve?

HTTP interceptors are middleware around `HttpClient`. They handle cross-cutting transport concerns while API services express domain-specific endpoints.
...
Service says what endpoint; interceptor says what every HTTP trip must carry. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Requests pass through the interceptor chain outward
2. responses/errors pass back inward. Functional interceptors can clone immutable requests, add headers, transform errors, and call the next handler.
...
An auth interceptor adds an access token; a correlation interceptor adds a request ID; an error interceptor maps transport failures. `PaymentApi` still defines `createPayment`.
...
**Use it when:** For auth headers, correlation, safe error normalization, telemetry, or caching policy applied broadly.

**Avoid or reconsider it when:** For endpoint-specific business rules or UI messages that need feature context.
...
Interceptors are like ASP.NET Core middleware but only for Angular HttpClient calls. A service owns a domain API; a guard owns navigation; an interceptor owns transport-wide policy.
...
- refreshing tokens with recursive loops
- swallowing errors
- spinner counter races
...
A payment screen uses **How do interceptors fit with services**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** HTTP interceptors are middleware around `HttpClient`. They handle cross-cutting transport concerns while API services express domain-specific endpoints.
- **Remember:** Service says what endpoint; interceptor says what every HTTP trip must carry.
