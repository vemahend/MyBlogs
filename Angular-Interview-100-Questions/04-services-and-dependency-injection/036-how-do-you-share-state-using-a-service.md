...
Share state through a service by exposing readonly signals/observables and explicit command methods. The provider scope must match the intended lifetime. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Hide the pen; share the paper. Consumers read state and request changes.**

## 1. What problem does it solve?

Share state through a service by exposing readonly signals/observables and explicit command methods. The provider scope must match the intended lifetime.
...
Hide the pen; share the paper. Consumers read state and request changes. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. A private writable signal or subject owns mutations. Derived state uses `computed` or operators. Consumers read a readonly view
2. commands enforce invariants before changing state.
...
A route-scoped payment draft store holds selected source account, beneficiary, and amount while the user moves between steps. Sensitive details are not persisted casually.
...
```ts
@Injectable() export class DraftStore { private draft=signal(emptyDraft); readonly value=this.draft.asReadonly(); patch(v:Partial<Draft>){this.draft.update(x=>({...x,...v}))} }
```
...
**Use it when:** For multiple consumers with a clear owner and lifetime.

**Avoid or reconsider it when:** For direct parent-child state or server-authoritative data that should simply be refetched.
...
Signals are excellent for synchronous UI state; RxJS for asynchronous streams; URL for shareable state; NgRx-like stores for complex global workflows.
...
- arbitrary setters
- root scope for wizard data
- state lost on refresh without decision
- localStorage of banking secrets.
...
A payment screen uses **How do you share state using a service**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Share state through a service by exposing readonly signals/observables and explicit command methods. The provider scope must match the intended lifetime.
- **Remember:** Hide the pen; share the paper. Consumers read state and request changes.
