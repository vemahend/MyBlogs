# How do you lazy load modules or standalone routes?
...
Lazy-load modules or standalone routes by placing dynamic imports at feature route boundaries so their code is downloaded only when navigation needs it. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **A lazy boundary must also be a dependency boundary.**

## 1. What problem does it solve?

Lazy-load modules or standalone routes by placing dynamic imports at feature route boundaries so their code is downloaded only when navigation needs it.
...
A lazy boundary must also be a dependency boundary. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. `loadComponent` loads a standalone page
2. `loadChildren` loads child route configuration/module. The bundler creates chunks, router fetches them, and optional preloading warms likely routes.
...
Admin payment reconciliation is a lazy standalone route with route-scoped providers; ordinary customers never download it.
...
```ts
{path:'reconciliation',providers:[ReconciliationStore],loadChildren:()=>import('./reconciliation.routes').then(m=>m.routes)}
```
...
**Use it when:** For non-critical feature areas and role-specific/admin screens.

**Avoid or reconsider it when:** For tiny landing route or code required immediately after boot.
...
Route lazy loading defers whole features. `@defer` defers components inside a rendered template. Code splitting can be defeated by eager imports.
...
- Shared index/barrel eagerly imports feature
- giant vendor chunk
- no preloading strategy
- auth guard itself imports admin code
- route providers wrong scope.
...
A payment screen uses **How do you lazy load modules or standalone routes**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Lazy-load modules or standalone routes by placing dynamic imports at feature route boundaries so their code is downloaded only when navigation needs it.
- **Remember:** A lazy boundary must also be a dependency boundary.
