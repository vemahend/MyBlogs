...
Avoid subscription memory leaks by preferring async pipe/toSignal for rendering, using `takeUntilDestroyed` for imperative long-lived streams, and cleaning timers, listeners, sockets, and third-party resources. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **After leaving the page, nothing from that page should still be listening.**

## 1. What problem does it solve?

Avoid subscription memory leaks by preferring async pipe/toSignal for rendering, using `takeUntilDestroyed` for imperative long-lived streams, and cleaning timers, listeners, sockets, and third-party resources.
...
After leaving the page, nothing from that page should still be listening. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. A leak occurs when a live producer retains a callback/component graph after the view is destroyed. Cleanup must be tied to the correct injection/lifecycle scope, including inner subscriptions created by flattening.
...
Navigate repeatedly into and out of live-balance view, then verify subscriber/listener counts and heap snapshots return near baseline.
...
```ts
ngOnInit(){this.ticks$.pipe(takeUntilDestroyed()).subscribe()}
ngOnDestroy(){clearInterval(this.timer);this.chart?.dispose();}
```
...
**Use it when:** Always audit long-lived sources and non-Angular integrations.

**Avoid or reconsider it when:** Do not blindly unsubscribe shared application services that intentionally outlive a component.
...
Memory leak differs from duplicate subscription and from slow change detection, though all may worsen over navigation. Finite HTTP usually completes; WebSocket never does naturally.
...
- Root service intentionally lives forever but holds component callback
- event listener removal mismatch
- interval not cleared
- shareReplay cache retains huge object
- inner stream survives.
...
A payment screen uses **How do you avoid memory leaks from subscriptions**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Avoid subscription memory leaks by preferring async pipe/toSignal for rendering, using `takeUntilDestroyed` for imperative long-lived streams, and cleaning timers, listeners, sockets, and third-party resources.
- **Remember:** After leaving the page, nothing from that page should still be listening.
