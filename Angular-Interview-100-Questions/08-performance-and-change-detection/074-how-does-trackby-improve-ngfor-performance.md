...
Tracking improves repeated-list performance by preserving view/component identity for unchanged items instead of recreating them when collection references or order change. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Identity survives refresh; position does not.**

## 1. What problem does it solve?

Tracking improves repeated-list performance by preserving view/component identity for unchanged items instead of recreating them when collection references or order change.
...
Identity survives refresh; position does not. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Angular’s differ uses the track key to match old and new records. It can move an existing view, preserve focus/local state, and update only changed bindings.
...
A polling transaction list returns new object instances every 10 seconds. Tracking by immutable transaction ID keeps row menus and focus stable.
...
```ts
@for(tx of transactions();track tx.id){<app-transaction-row [tx]="tx"/>}
```
...
**Use it when:** For collections refreshed, sorted, filtered, inserted, or removed.

**Avoid or reconsider it when:** A static list may not show measurable gain, but stable identity remains correct.
...
Tracking does not reduce API payload, array transformation, or the number of currently rendered rows. Combine with pagination/virtualization for scale.
...
- Duplicate key
- index on sortable list
- mutable key
- key calculation expensive
- expecting tracking to fix slow cell components.
...
- **Definition:** Tracking improves repeated-list performance by preserving view/component identity for unchanged items instead of recreating them when collection references or order change.
- **Remember:** Identity survives refresh; position does not.
