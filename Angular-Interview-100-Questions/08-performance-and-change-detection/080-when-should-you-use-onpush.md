...
Use OnPush when component inputs/state follow explicit immutable, Observable, or signal-based update paths and subtree-check reduction is useful. Verify current Angular defaults before adding boilerplate. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Choose OnPush with an ownership model, not as a magic speed flag.**

## 1. What problem does it solve?

Use OnPush when component inputs/state follow explicit immutable, Observable, or signal-based update paths and subtree-check reduction is useful. Verify current Angular defaults before adding boilerplate.
...
Choose OnPush with an ownership model, not as a magic speed flag. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. The component is revisited when a recognized trigger marks it. Event handlers, async pipe, signal reads, and changed input references establish those triggers.
...
Use it for repeated transaction rows and dashboard widgets with clear VMs; measure before/after in the real interaction.
...
```ts
@Component({changeDetection:ChangeDetectionStrategy.OnPush,templateUrl:'./row.html'})
export class TransactionRow {row=input.required<TransactionVm>();}
```
...
**Use it when:** For component trees with predictable state and measured/reasonable performance benefits.

**Avoid or reconsider it when:** For code with uncontrolled mutation until the state model is corrected first.
...
OnPush complements signals, track identity, and component boundaries. It does not replace server pagination, virtualization, or efficient template expressions.
...
- Input mutation
- hidden global callbacks
- stale third-party widget
- manual subscription not reflected
- no performance measurement
- unnecessary markForCheck chains.
...
- **Definition:** Use OnPush when component inputs/state follow explicit immutable, Observable, or signal-based update paths and subtree-check reduction is useful. Verify current Angular defaults before adding boilerplate.
- **Remember:** Choose OnPush with an ownership model, not as a magic speed flag.
