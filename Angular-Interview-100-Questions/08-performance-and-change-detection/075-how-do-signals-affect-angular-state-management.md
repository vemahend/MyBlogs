# How do signals affect Angular state management?
...
Signals provide fine-grained synchronous reactive state through writable signals, computed derivations, and effects. They can simplify local/store state and integrate with Angular rendering. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Signal is a cell; computed is a formula; effect is an alarm.**

## 1. What problem does it solve?

Signals provide fine-grained synchronous reactive state through writable signals, computed derivations, and effects. They can simplify local/store state and integrate with Angular rendering.
...
Signal is a cell; computed is a formula; effect is an alarm. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Reading a signal registers a dependency. Updating it invalidates dependent computed values and marks consuming views. Computed values are lazy/memoized
2. effects are for side effects, not copying state.
...
A payment store holds draft signal, computed total/validity, and exposes command methods. RxJS handles HTTP/search streams and converts at deliberate boundaries.
...
```ts
private draft=signal(emptyDraft);
readonly total=computed(()=>this.draft().items.reduce((n,x)=>n+x.amount,0));
readonly vm=toSignal(this.api.load(),{initialValue:null});
```
...
**Use it when:** For local and shared synchronous state with clear derivation and ownership.

**Avoid or reconsider it when:** For complex event-time orchestration already expressed well in RxJS, or as a reason to remove all Observables.
...
Signals store current state; Observables model time/events/async streams; computed derives state; effect performs side effects; a full store adds conventions/devtools.
...
- Effect used to propagate state causing loops
- mutation without set/update
- converting repeatedly
- global signals with unclear lifetime
...
A payment screen uses **How do signals affect Angular state management**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Signals provide fine-grained synchronous reactive state through writable signals, computed derivations, and effects. They can simplify local/store state and integrate with Angular rendering.
- **Remember:** Signal is a cell; computed is a formula; effect is an alarm.
