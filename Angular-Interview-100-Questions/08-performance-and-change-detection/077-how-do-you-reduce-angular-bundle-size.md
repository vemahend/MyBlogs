...
Reduce bundle size by measuring bundles, removing unused/heavy dependencies, lazy/deferred loading, using tree-shakable providers/imports, and replacing expensive libraries with focused alternatives where justified. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **First measure who is in the bundle; then decide who should arrive later or not at all.**

## 1. What problem does it solve?

Reduce bundle size by measuring bundles, removing unused/heavy dependencies, lazy/deferred loading, using tree-shakable providers/imports, and replacing expensive libraries with focused alternatives where justified.
...
First measure who is in the bundle; then decide who should arrive later or not at all. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Tree shaking removes statically unreachable code
2. dynamic imports split chunks
3. minification/compression reduce transfer. CommonJS and side-effectful packages can block optimization. Runtime and transfer sizes are different.
...
Replace a full date library used for one format, lazy-load report export, defer charts, and keep payment entry shell minimal.
...
```ts
@defer (on viewport) { <app-heavy-chart [data]="data()"/> } @placeholder { <div class="chart-skeleton"></div> }
```
...
**Use it when:** When performance budget or analysis identifies startup/download/parse cost.

**Avoid or reconsider it when:** Do not remove well-supported libraries for tiny savings that increase security/maintenance risk.
...
Initial bundle affects startup; lazy chunks affect later navigation; payload data is not JS bundle; image/font optimization is separate but equally important.
...
- Importing entire utility library
- duplicate versions
- source maps shipped publicly by mistake
- lazy chunk still huge
- chasing bytes while API dominates.
...
A payment screen uses **How do you reduce Angular bundle size**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Reduce bundle size by measuring bundles, removing unused/heavy dependencies, lazy/deferred loading, using tree-shakable providers/imports, and replacing expensive libraries with focused alternatives where justified.
- **Remember:** First measure who is in the bundle; then decide who should arrive later or not at all.
