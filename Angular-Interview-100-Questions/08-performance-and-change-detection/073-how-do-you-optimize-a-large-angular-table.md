# How do you optimize a large Angular table?
...
Optimize a large table across data, DOM, rendering, interaction, and backend query layers. Usually the biggest win is rendering fewer rows and fetching only needed data. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Do not render what the user cannot see or use yet.**

## 1. What problem does it solve?

Optimize a large table across data, DOM, rendering, interaction, and backend query layers. Usually the biggest win is rendering fewer rows and fetching only needed data.
...
Do not render what the user cannot see or use yet. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Server pagination/filter/sort reduces payload
2. virtual scrolling limits live DOM
3. stable tracking reuses rows
4. OnPush/signals limit checks
5. memoized/cached formatting avoids repeated work.
...
A transaction table uses cursor pagination, server-side filters, virtual rows, stable `transactionId`, accessible sticky headers, and row expansion loaded on demand.
...
```ts
@for(tx of visibleTransactions();track tx.id){<app-transaction-row [tx]="tx"/>}
// Add server pagination and virtual scrolling for large data.
```
...
**Use it when:** For datasets large enough to affect payload, DOM, or interaction latency.

**Avoid or reconsider it when:** For small tables where added complexity harms accessibility and maintainability.
...
Pagination limits data per request; virtualization limits DOM; track identity reuses DOM; OnPush limits subtree work. They solve different bottlenecks.
...
- Client sorting 100k rows
- one form control per cell
- impure pipes
- track by index
- expensive components in each row
- inaccessible virtualization.
...
A payment screen uses **How do you optimize a large Angular table**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Optimize a large table across data, DOM, rendering, interaction, and backend query layers. Usually the biggest win is rendering fewer rows and fetching only needed data.
- **Remember:** Do not render what the user cannot see or use yet.
