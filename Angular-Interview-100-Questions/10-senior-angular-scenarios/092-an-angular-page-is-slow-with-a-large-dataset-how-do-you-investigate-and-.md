...
Investigate a slow large-data page by establishing a production-like baseline, measuring network/CPU/DOM/memory/change detection, and applying the fix to the proven bottleneck. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Count data, DOM nodes, checks, requests, and milliseconds—then reduce the biggest number.**

## 1. What problem does it solve?

Investigate a slow large-data page by establishing a production-like baseline, measuring network/CPU/DOM/memory/change detection, and applying the fix to the proven bottleneck.
...
Count data, DOM nodes, checks, requests, and milliseconds—then reduce the biggest number. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Large datasets multiply every per-row cost. Payload parsing, mapping, DOM creation, template calls, layout, and subscriptions can each dominate
2. one optimization cannot solve all layers.
...
Measure a 50k transaction view, then use server pagination/filtering, virtualization, stable tracking, lightweight rows, cached pure formatting, and accessible interaction.
...
```ts
// Measure: Network payload/TTFB, long tasks/layout, Angular profile, DOM count and heap after repeated navigation.
```
...
**Use it when:** Use performance budgets and compare traces before/after each change.

**Avoid or reconsider it when:** Do not rewrite state management or add memoization until evidence points there.
...
OnPush reduces checks; track preserves identity; virtualization reduces DOM; pagination reduces data; indexes/query design reduce backend latency.
...
- Loading all rows then hiding
- performance test with tiny fixture
- impure pipe per cell
- memory leak after paging
- accessibility lost in virtual table.
...
A payment screen uses **An Angular page is slow with a large dataset. How do you investigate and fix it**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Investigate a slow large-data page by establishing a production-like baseline, measuring network/CPU/DOM/memory/change detection, and applying the fix to the proven bottleneck.
- **Remember:** Count data, DOM nodes, checks, requests, and milliseconds—then reduce the biggest number.
