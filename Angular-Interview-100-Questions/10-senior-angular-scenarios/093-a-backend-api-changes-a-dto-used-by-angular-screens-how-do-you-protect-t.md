...
Protect the frontend from backend DTO change by treating the HTTP boundary as untrusted/versioned: generate or define transport types, validate important runtime payloads, map to stable UI/domain models, and run contract tests. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **DTO stops at the door; the UI speaks its own stable model.**

## 1. What problem does it solve?

Protect the frontend from backend DTO change by treating the HTTP boundary as untrusted/versioned: generate or define transport types, validate important runtime payloads, map to stable UI/domain models, and run contract tests.
...
DTO stops at the door; the UI speaks its own stable model. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. TypeScript compilation cannot detect a deployed server shape change. A boundary adapter can parse/version/default/reject fields so breakage is contained instead of spreading through components.
...
If `amount` changes representation or `status` gains values, the mapper converts safely or produces a controlled incompatible-response state with telemetry.
...
```ts
const dto=PaymentSchema.parse(raw);
return toPaymentVm(dto); // raw transport DTO stops at the boundary
```
...
**Use it when:** For every externally controlled API, with stricter checks on money, authorization, and workflow status.

**Avoid or reconsider it when:** Avoid duplicating mapping layers for trivial internal data if versioned deployment and risk truly justify direct use.
...
Compile-time types prevent local misuse; runtime schemas validate actual JSON; consumer-driven/OpenAPI contract tests detect compatibility before release; API versioning manages intentional breaks.
...
- Raw DTO everywhere
- enum exhaustive switch crashes on new value
- optional field silently assumed
- generated client trusted as runtime validation
...
A payment screen uses **A backend API changes a DTO used by Angular screens. How do you protect the frontend**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Protect the frontend from backend DTO change by treating the HTTP boundary as untrusted/versioned: generate or define transport types, validate important runtime payloads, map to stable UI/domain models, and run contract tests.
- **Remember:** DTO stops at the door; the UI speaks its own stable model.
