# A role-based menu shows links the user cannot access. How do you fix it?
...
A role-based menu and route access must derive from the same centralized capability policy, while the API independently authorizes the operation. Avoid separate hard-coded role checks in menu and guards. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **One policy feeds UI and routing; API remains final authority.**

## 1. What problem does it solve?

A role-based menu and route access must derive from the same centralized capability policy, while the API independently authorizes the operation. Avoid separate hard-coded role checks in menu and guards.
...
One policy feeds UI and routing; API remains final authority. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Normalize identity/claims into capabilities, expose readonly policy state, and let both navigation model and guards query it. Refresh token/claim changes must update consistently.
...
`canApprovePayment` controls the Approvals menu and route UX; the approve endpoint checks the same business permission server-side and returns 403 if unauthorized.
...
```ts
readonly menu=computed(()=>NAV_ITEMS.filter(x=>this.policy.has(x.requiredCapability)));
```
...
**Use it when:** Centralize capability evaluation, test the matrix, and handle 403 even after menu/guard checks.

**Avoid or reconsider it when:** Do not duplicate backend policy engine blindly in frontend or expose sensitive authorization data unnecessarily.
...
Roles are identity groupings; permissions/capabilities express actions; feature flags control rollout, not authorization; menu visibility is discoverability.
...
- Menu uses role string A while guard uses B
- stale cached claims
- hidden link treated secure
- deep link behavior inconsistent
- tenant/resource permission ignored.
...
A payment screen uses **A role-based menu shows links the user cannot access. How do you fix it**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** A role-based menu and route access must derive from the same centralized capability policy, while the API independently authorizes the operation. Avoid separate hard-coded role checks in menu and guards.
- **Remember:** One policy feeds UI and routing; API remains final authority.
