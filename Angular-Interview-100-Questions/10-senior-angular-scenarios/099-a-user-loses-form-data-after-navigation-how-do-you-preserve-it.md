# A user loses form data after navigation. How do you preserve it?
...
Preserve form data based on required lifetime and sensitivity: route-scoped draft service for navigation, URL only for safe/shareable fields, session storage for deliberate refresh survival, or backend drafts for cross-device/reliable recovery. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Choose preservation by lifetime: component, route, refresh, session, or device.**

## 1. What problem does it solve?

Preserve form data based on required lifetime and sensitivity: route-scoped draft service for navigation, URL only for safe/shareable fields, session storage for deliberate refresh survival, or backend drafts for cross-device/reliable recovery.
...
Choose preservation by lifetime: component, route, refresh, session, or device. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Capture form changes with controlled debounce and map to a draft model. Restore before the user edits, version the schema, clear on submit/logout/expiry, and avoid feedback loops.
...
A multi-step transfer keeps non-sensitive draft in route store; a saved beneficiary/payment draft may go to encrypted authenticated backend. Never casually persist full account/security data in localStorage.
...
```ts
form.valueChanges.pipe(debounceTime(500),takeUntilDestroyed()).subscribe(v=>draftStore.save(toSafeDraft(v)));
```
...
**Use it when:** When UX/risk requires recovery, with explicit privacy, expiry, conflict, and cleanup rules.

**Avoid or reconsider it when:** For sensitive one-time values that policy says must not persist.
...
CanDeactivate prevents accidental loss; persistence recovers after loss; store preserves during component changes; backend draft survives device/session.
...
- Persisting secrets
- stale draft overwrites server/current edit
- no schema version
- every keystroke network write
- draft not cleared after success
...
A payment screen uses **A user loses form data after navigation. How do you preserve it**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Preserve form data based on required lifetime and sensitivity: route-scoped draft service for navigation, URL only for safe/shareable fields, session storage for deliberate refresh survival, or backend drafts for cross-device/reliable recovery.
- **Remember:** Choose preservation by lifetime: component, route, refresh, session, or device.
