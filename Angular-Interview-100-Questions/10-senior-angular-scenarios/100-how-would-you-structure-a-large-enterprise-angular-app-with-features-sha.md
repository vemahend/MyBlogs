# How would you structure a large enterprise Angular app with features, shared UI, services, and routing?
...
Structure a large Angular app around business features with clear dependency direction: app shell/platform, lazy feature areas, feature data-access/state/application/UI, and a small truly shared design system. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Organize by why the business changes, then layer inside the feature where useful.**

## 1. What problem does it solve?

Structure a large Angular app around business features with clear dependency direction: app shell/platform, lazy feature areas, feature data-access/state/application/UI, and a small truly shared design system.
...
Organize by why the business changes, then layer inside the feature where useful. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Routes create loading and provider boundaries. Standalone components keep imports explicit. Public APIs/barrels and lint rules prevent feature cycles. Cross-cutting auth/config/telemetry live at platform level.
...
`payments`, `accounts`, `reconciliation`, and `admin` are lazy features. Payments contains pages, UI, facade/store, API/mappers, and route config. Shared UI contains button/table primitives, not payment rules.
...
src/app/
  platform/
  shared/ui/
  features/payments/{pages,ui,data-access,state,application}
  features/accounts/
  app.routes.ts
```
...
**Use it when:** For enterprise scale, enforce boundaries gradually with architecture tests/linting, ownership, and consistent patterns.

**Avoid or reconsider it when:** Do not introduce micro-frontends, full store machinery, or many layers without team/deployment complexity that justifies them.
...
Feature-based structure optimizes ownership/change; layer-only structure scatters a feature. Micro-frontends add independent deployment cost and are not the default solution.
...
- Global components/services folders
- shared becomes dumping ground
- features import each other
- root state for everything
- raw DTO crosses app
- route/provider lifetimes ignored.
...
A payment screen uses **How would you structure a large enterprise Angular app with features, shared UI, services, and routing**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Structure a large Angular app around business features with clear dependency direction: app shell/platform, lazy feature areas, feature data-access/state/application/UI, and a small truly shared design system.
- **Remember:** Organize by why the business changes, then layer inside the feature where useful.
