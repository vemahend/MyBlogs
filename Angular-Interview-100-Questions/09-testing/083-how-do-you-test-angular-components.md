...
Test components through their public behavior: set inputs, trigger user actions, run synchronization, and assert accessible DOM/output/service interaction. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Test what the user sees and can do, not private method choreography.**

## 1. What problem does it solve?

Test components through their public behavior: set inputs, trigger user actions, run synchronization, and assert accessible DOM/output/service interaction.
...
Test what the user sees and can do, not private method choreography. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. TestBed creates the dependency environment
2. a fixture owns the instance and rendered DOM. Change detection is not automatically synchronous after every setup operation, so tests must stabilize deliberately.
...
Given an invalid amount, Pay is disabled and an accessible message appears; given valid input, one submit output/API command occurs despite rapid click.
...
fixture.nativeElement.querySelector('button[type=submit]').click();fixture.detectChanges();
```
...
**Use it when:** For UI state, input/output contracts, accessibility, event handling, and service integration.

**Avoid or reconsider it when:** Keep pure calculation tests outside TestBed when Angular rendering adds no value.
...
Class-only tests cover pure methods; component tests cover template integration; harnesses provide stable interaction APIs; E2E covers browser/system integration.
...
- CSS selectors tied to markup
- no detect/stability handling
- shallow mocks hiding integration
- only happy path.
...
- **Definition:** Test components through their public behavior: set inputs, trigger user actions, run synchronization, and assert accessible DOM/output/service interaction.
- **Remember:** Test what the user sees and can do, not private method choreography.
