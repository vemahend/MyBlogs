...
Test services according to responsibility: pure services as ordinary classes, DI services with TestBed, API clients with HTTP testing, and state services through public commands/read models. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Service test should prove its public promise, including time and error behavior.**

## 1. What problem does it solve?

Test services according to responsibility: pure services as ordinary classes, DI services with TestBed, API clients with HTTP testing, and state services through public commands/read models.
...
Service test should prove its public promise, including time and error behavior. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. The smallest setup gives fastest feedback. Control dependencies such as clock, UUID, storage, and HTTP using provider replacements
2. test observable timelines when ordering matters.
...
Test payment facade transitions idle→submitting→success/error, preserves idempotency key during retry, and prevents stale response overwriting current state.
...
```ts
const api=jasmine.createSpyObj<PaymentApi>('PaymentApi',['create']);api.create.and.returnValue(of(payment));
const facade=new PaymentFacade(api,clock,uuid);
```
...
**Use it when:** For domain orchestration, state transitions, mapping, errors, and side effects.

**Avoid or reconsider it when:** Do not test Angular/HttpClient itself; test your configuration and behavior.
...
A unit test isolates collaborators; integration test includes several real services; contract test validates API shape.
...
- Overusing TestBed
- subscription assertions before emission
- flaky real timers
- public Subject mutated by test
- exact internal call sequence asserted.
...
A payment screen uses **How do you test Angular services**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Test services according to responsibility: pure services as ordinary classes, DI services with TestBed, API clients with HTTP testing, and state services through public commands/read models.
- **Remember:** Service test should prove its public promise, including time and error behavior.
