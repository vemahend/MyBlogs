# How do you test components with @Input and @Output?
...
Test input/output components by setting inputs through Angular’s binding-aware test APIs/host, synchronizing the fixture, subscribing or hosting the output, triggering a user action, and asserting the semantic event. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Arrange input, act like user, assert output and DOM.**

## 1. What problem does it solve?

Test input/output components by setting inputs through Angular’s binding-aware test APIs/host, synchronizing the fixture, subscribing or hosting the output, triggering a user action, and asserting the semantic event.
...
Arrange input, act like user, assert output and DOM. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Angular-managed input setting more accurately exercises input lifecycle than direct field assignment. Output assertions should verify payload and timing without relying on private emitter internals.
...
Pass a pending Payment VM; verify Approve is visible; click it; assert `approveRequested` emits the immutable payment ID once.
...
```ts
fixture.componentRef.setInput('payment',pendingPayment);fixture.detectChanges();
fixture.componentInstance.approveRequested.subscribe(x=>emitted.push(x));clickApprove();
```
...
**Use it when:** For every reusable component contract and important user intent.

**Avoid or reconsider it when:** Do not assert outputs that users cannot actually trigger through the UI unless it is a programmatic component API.
...
A host test verifies real template binding; direct fixture test is smaller; harness abstracts DOM interaction for reusable libraries.
...
- setting input after detect without update
- output name collision
- asserting EventEmitter implementation
- mutating same input reference.
...
- **Definition:** Test input/output components by setting inputs through Angular’s binding-aware test APIs/host, synchronizing the fixture, subscribing or hosting the output, triggering a user action, and asserting the semantic event.
- **Remember:** Arrange input, act like user, assert output and DOM.
