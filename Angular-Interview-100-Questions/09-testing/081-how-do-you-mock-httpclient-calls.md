...
Mock HttpClient calls with Angular’s HTTP testing provider/controller, which captures requests and lets the test assert method, URL, headers, body, cancellation, and then flush success or error. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Mock one boundary at a time: component→service or service→HTTP.**

## 1. What problem does it solve?

Mock HttpClient calls with Angular’s HTTP testing provider/controller, which captures requests and lets the test assert method, URL, headers, body, cancellation, and then flush success or error.
...
Mock one boundary at a time: component→service or service→HTTP. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. The testing backend replaces real transport. The service subscribes normally
2. `expectOne` finds a request
3. `flush` sends a controlled response
4. `verify` catches unexpected outstanding requests.
...
Test that `PaymentApi.create` sends the correct DTO and idempotency header, maps 409 conflict, and never reaches a real environment.
...
const req=http.expectOne('/api/payments');expect(req.request.method).toBe('POST');req.flush(response);http.verify();
```
...
**Use it when:** For unit/integration tests of Angular HTTP clients and interceptors.

**Avoid or reconsider it when:** Do not use it to prove the backend implements the contract.
...
Mocking the API service tests a component; HttpTestingController tests the API service’s HTTP contract. End-to-end tests verify integration with a real/stub server.
...
- Testing implementation URL fragments too rigidly
- no verify
- forgetting subscribe
- incorrect provider order
- only success
- tokens/secrets hard-coded.
...
A payment screen uses **How do you mock HttpClient calls**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Mock HttpClient calls with Angular’s HTTP testing provider/controller, which captures requests and lets the test assert method, URL, headers, body, cancellation, and then flush success or error.
- **Remember:** Mock one boundary at a time: component→service or service→HTTP.
