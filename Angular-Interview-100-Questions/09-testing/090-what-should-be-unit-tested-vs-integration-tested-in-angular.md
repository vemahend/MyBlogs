# What should be unit tested vs integration tested in Angular?
...
Unit-test pure rules and isolated state transitions; integration-test Angular wiring and collaborating boundaries. Choose the lowest-cost layer that can reliably expose the risk. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Test logic alone when possible; test wiring together when the wiring is the risk.**

## 1. What problem does it solve?

Unit-test pure rules and isolated state transitions; integration-test Angular wiring and collaborating boundaries. Choose the lowest-cost layer that can reliably expose the risk.
...
Test logic alone when possible; test wiring together when the wiring is the risk. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Unit tests localize failures and run fast. Integration tests catch template/DI/router/forms/interceptor interactions. Contract/E2E tests cover cross-process assumptions and critical journeys.
...
Unit-test amount/DTO mapper; component-test form messages and duplicate click; HTTP-test request contract; E2E-test a critical transfer with controlled backend.
...
```ts
// Unit: mapper/validator/state transition. Integration: form+template+facade. E2E: critical successful/declined payment.
```
...
**Use it when:** Use a risk matrix: financial impact, change frequency, integration uncertainty, and execution cost.

**Avoid or reconsider it when:** Do not enforce a rigid ratio that ignores product risk.
...
A private method is not a testing category. Behavior, risk, and boundary determine the layer.
...
- Unit testing framework internals
- integration testing every edge permutation
- mocked contract drifting from API
- duplicated assertions at all layers.
...
- **Definition:** Unit-test pure rules and isolated state transitions; integration-test Angular wiring and collaborating boundaries. Choose the lowest-cost layer that can reliably expose the risk.
- **Remember:** Test logic alone when possible; test wiring together when the wiring is the risk.
