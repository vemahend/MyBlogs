...
Structure frontend tests as a portfolio: many fast unit tests for pure logic, focused component tests for behavior, integration tests for Angular boundaries, contract tests for DTOs, and a small critical end-to-end suite. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **More realism costs speed and diagnosis; place each risk at the cheapest reliable layer.**

## 1. What problem does it solve?

Structure frontend tests as a portfolio: many fast unit tests for pure logic, focused component tests for behavior, integration tests for Angular boundaries, contract tests for DTOs, and a small critical end-to-end suite.
...
More realism costs speed and diagnosis; place each risk at the cheapest reliable layer. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Test layers trade speed, realism, and diagnostic precision. CI should shard appropriately, control time/network, and retain failure artifacts. Ownership follows feature folders.
...
Payments get dense tests for validation/idempotency/error mapping, component integration for submission, contract tests against API schema, and E2E for one successful/declined flow.
...
```ts
// Unit validators/mappers → component integration → HTTP/contract → a few critical E2E journeys.
```
...
**Use it when:** For any large app: define risk-based layers, conventions, builders, and CI ownership.

**Avoid or reconsider it when:** Avoid chasing coverage percentage without meaningful assertions.
...
Testing pyramid is a heuristic; frontend often benefits from a testing trophy emphasizing component integration. The correct mix depends on risk.
...
- Every test through E2E
- snapshot-only tests
- shared global fixtures
- no contract tests
- brittle selectors
- tests organized separately from features.
...
- **Definition:** Structure frontend tests as a portfolio: many fast unit tests for pure logic, focused component tests for behavior, integration tests for Angular boundaries, contract tests for DTOs, and a small critical end-to-end suite.
- **Remember:** More realism costs speed and diagnosis; place each risk at the cheapest reliable layer.
