...
An Observable is a description of values over time. It does nothing until subscribed unless it wraps a hot source, and operators create a new Observable rather than mutating the old one. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Observable is a recipe for a timeline; subscription starts listening/cooking.**

## 1. What problem does it solve?

An Observable is a description of values over time. It does nothing until subscribed unless it wraps a hot source, and operators create a new Observable rather than mutating the old one.
...
Observable is a recipe for a timeline; subscription starts listening/cooking. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Subscription invokes the producer
2. notifications travel through the operator chain
3. teardown runs on unsubscribe/complete/error. Cold sources are per subscriber, hot sources are shared independently.
...
Represent route changes, form values, balance updates, and HTTP responses. Compose them instead of nesting subscriptions.
...
```ts
const result$=new Observable<Result>(subscriber=>{const stop=startWork(v=>subscriber.next(v));return ()=>stop();});
```
...
**Use it when:** For asynchronous events, multiple values, cancellation, and compositional pipelines.

**Avoid or reconsider it when:** For simple synchronous state where a signal/value is clearer.
...
Subject is both observer and Observable and is hot; BehaviorSubject retains a latest value; signal stores current state; Promise emits once.
...
- public subjects
- side effects in map
- cold HTTP duplicated
- no error path
- long-lived stream never cleaned.
...
- **Definition:** An Observable is a description of values over time. It does nothing until subscribed unless it wraps a hot source, and operators create a new Observable rather than mutating the old one.
- **Remember:** Observable is a recipe for a timeline; subscription starts listening/cooking.
