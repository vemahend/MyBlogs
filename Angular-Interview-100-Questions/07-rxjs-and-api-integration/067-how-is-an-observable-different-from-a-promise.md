...
A Promise represents one eventual result and starts eagerly; an Observable represents a lazy/cancellable stream that may emit zero, one, or many values and can be transformed with operators. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Promise: one future box. Observable: a controllable timeline.**

## 1. What problem does it solve?

A Promise represents one eventual result and starts eagerly; an Observable represents a lazy/cancellable stream that may emit zero, one, or many values and can be transformed with operators.
...
Promise: one future box. Observable: a controllable timeline. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Observables execute per subscription unless shared and provide next/error/complete channels. Unsubscription stops observation and may cancel underlying work. Promises cannot be cancelled natively once created.
...
HttpClient returns cold Observables; route parameters and form values are multi-value streams. `firstValueFrom` is appropriate only at a boundary requiring a Promise.
...
```ts
const once:Promise<Payment>=firstValueFrom(api.get(id));
const changing:Observable<ParamMap>=route.paramMap;
```
...
**Use it when:** Observable for Angular streams, HTTP composition, cancellation, multiple emissions.

**Avoid or reconsider it when:** Promise at APIs naturally modeled as one async result or when integrating Promise-only libraries.
...
Signals represent current synchronous reactive state; Observables represent events/time/async streams; Promises represent one future completion.
...
- Subscribing twice to cold HTTP
- converting everything to Promise and losing cancellation
- Observable never subscribed
- expecting unsubscribe to roll back server writes.
...
A payment screen uses **How is an Observable different from a Promise**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** A Promise represents one eventual result and starts eagerly; an Observable represents a lazy/cancellable stream that may emit zero, one, or many values and can be transformed with operators.
- **Remember:** Promise: one future box. Observable: a controllable timeline.
