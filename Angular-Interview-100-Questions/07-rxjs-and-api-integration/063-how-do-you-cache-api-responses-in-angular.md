# How do you cache API responses in Angular?
...
Cache API responses by deciding cache key, scope, freshness, invalidation, error behavior, and sharing semantics—not merely adding `shareReplay(1)`. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **A cache is a policy: key + lifetime + invalidation.**

## 1. What problem does it solve?

Cache API responses by deciding cache key, scope, freshness, invalidation, error behavior, and sharing semantics—not merely adding `shareReplay(1)`.
...
A cache is a policy: key + lifetime + invalidation. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. A service/store can memoize an Observable or normalized data by key. `shareReplay` shares a source subscription, but configuration, errors, refCount, and invalidation must be understood. HTTP/browser/CDN caching may be more appropriate.
...
Cache reference currencies for a session; cache payment detail briefly; invalidate or update it after approval. Never serve another user’s data from a shared server cache without correct partitioning.
...
```ts
private cache=new Map<string,Observable<Payment>>();
get(id:string){return this.cache.get(id)??this.http.get<Payment>('/api/payments/'+id).pipe(shareReplay({bufferSize:1,refCount:true}))}
```
...
**Use it when:** For read-heavy, stable data with explicit freshness and invalidation requirements.

**Avoid or reconsider it when:** For highly volatile balances/permissions unless the product tolerates staleness and refreshes correctly.
...
Memoization is in-memory app cache; HTTP caching uses headers; service worker supports offline assets/data strategies; backend cache is authoritative infrastructure.
...
- Permanent stale replay
- caching errors
- no user/tenant in key
...
- **Definition:** Cache API responses by deciding cache key, scope, freshness, invalidation, error behavior, and sharing semantics—not merely adding `shareReplay(1)`.
- **Remember:** A cache is a policy: key + lifetime + invalidation.
