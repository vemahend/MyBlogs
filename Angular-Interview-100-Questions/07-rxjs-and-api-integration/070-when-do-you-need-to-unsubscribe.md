...
Unsubscribe from streams that can outlive the consumer. You usually do not need manual cleanup for finite HttpClient calls, async pipe, or interop APIs that bind cleanup to a destroy context. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **If the source may live longer than the view, bind it to the view’s death.**

## 1. What problem does it solve?

Unsubscribe from streams that can outlive the consumer. You usually do not need manual cleanup for finite HttpClient calls, async pipe, or interop APIs that bind cleanup to a destroy context.
...
If the source may live longer than the view, bind it to the view’s death. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. Unsubscription invokes teardown and removes observers. `takeUntilDestroyed` completes a pipeline with the component/directive/service lifetime. Completion and unsubscription are related but not identical application signals.
...
Stop WebSocket balance updates and form value subscriptions on navigation. A one-response HttpClient request normally completes, though cancellation may still matter for stale views.
...
```ts
this.socket.messages$.pipe(takeUntilDestroyed()).subscribe(m=>this.apply(m));
// Prefer {{ balance$ | async }} for rendering.
```
...
**Use it when:** For long-lived streams, event sources, intervals, forms, sockets, router streams when manually subscribed.

**Avoid or reconsider it when:** Do not add noisy manual teardown to every finite stream; use lifecycle-aware patterns.
...
Async pipe/toSignal automate view-lifetime subscription; take(1) limits emissions; takeUntilDestroyed binds lifetime; manual Subscription aggregation is older and error-prone.
...
- Unsubscribing only some branches
- destroy Subject never completed
- takeUntil placed before inner operator incorrectly
- assuming service-root DestroyRef is component lifetime.
...
A payment screen uses **When do you need to unsubscribe**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Unsubscribe from streams that can outlive the consumer. You usually do not need manual cleanup for finite HttpClient calls, async pipe, or interop APIs that bind cleanup to a destroy context.
- **Remember:** If the source may live longer than the view, bind it to the view’s death.
