...
Handle API errors in the RxJS pipeline at the layer that has enough context to decide whether to recover, translate, retry, or rethrow. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Catch where you can make a meaningful decision; otherwise rethrow.**

## 1. What problem does it solve?

Handle API errors in the RxJS pipeline at the layer that has enough context to decide whether to recover, translate, retry, or rethrow.
...
Catch where you can make a meaningful decision; otherwise rethrow. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. `catchError` replaces the failed stream. Placement matters: inside switchMap keeps the outer input stream alive
2. outside may terminate it. `finalize` runs on complete, error, or unsubscribe.
...
Map 400 validation to fields, 401 to session handling, 403 to forbidden, 404 to resource state, 409 to concurrency conflict, and 5xx/network to retryable UI.
...
```ts
load$=id$.pipe(switchMap(id=>api.get(id).pipe(catchError(e=>e.status===404?of(null):throwError(()=>e)),finalize(()=>loading.set(false)))));
```
...
**Use it when:** For explicit recovery and safe error-state modeling at every async boundary.

**Avoid or reconsider it when:** Do not turn unknown failures into fake success/empty data.
...
Interceptor normalizes cross-cutting transport errors; API service maps DTO/protocol; facade chooses feature recovery; component displays user language.
...
- `catchError(() => EMPTY)` hiding failure
- retrying all status codes
- losing correlation ID
- loading flag not finalized
- outer stream permanently dead.
...
A payment screen uses **How do you handle API errors with RxJS**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Handle API errors in the RxJS pipeline at the layer that has enough context to decide whether to recover, translate, retry, or rethrow.
- **Remember:** Catch where you can make a meaningful decision; otherwise rethrow.
