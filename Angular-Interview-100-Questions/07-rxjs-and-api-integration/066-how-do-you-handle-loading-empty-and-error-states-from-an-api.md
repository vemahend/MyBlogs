# How do you handle loading, empty, and error states from an API?
...
Model loading, empty, data, and error as explicit mutually understandable UI states rather than several booleans that can contradict each other. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Empty is data; error is failure; do not confuse them.**

## 1. What problem does it solve?

Model loading, empty, data, and error as explicit mutually understandable UI states rather than several booleans that can contradict each other.
...
Empty is data; error is failure; do not confuse them. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. A discriminated union, resource abstraction, or derived state makes valid transitions clear. RxJS can map materialized outcomes
2. signals/computed can derive the current view.
...
A transaction page differentiates first load, refresh-with-old-data, no transactions, forbidden, transient failure, and loaded results.
...
```ts
type LoadState<T>={kind:'loading'}|{kind:'empty'}|{kind:'ready';data:T}|{kind:'error';message:string};
readonly state=signal<LoadState<Payment[]>>({kind:'loading'});
```
...
**Use it when:** For every API-driven screen, with a state model appropriate to its complexity.

**Avoid or reconsider it when:** Avoid a heavy state machine for a trivial one-shot value if clear template branches suffice.
...
Empty is a successful response with no items; error is a failed attempt; loading may coexist with stale data during refresh if modeled intentionally.
...
- Three booleans producing loading+error+data simultaneously
- clearing useful data on refresh
- spinner forever after cancellation
- generic error for 403.
...
A payment screen uses **How do you handle loading, empty, and error states from an API**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Model loading, empty, data, and error as explicit mutually understandable UI states rather than several booleans that can contradict each other.
- **Remember:** Empty is data; error is failure; do not confuse them.
