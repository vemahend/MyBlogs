...
The async pipe subscribes to an Observable or Promise in a template, renders the latest value, marks the view for update, and automatically unsubscribes when the view is destroyed or the expression reference changes. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Async pipe: subscribe, display, cleanup.**

## 1. What problem does it solve?

The async pipe subscribes to an Observable or Promise in a template, renders the latest value, marks the view for update, and automatically unsubscribes when the view is destroyed or the expression reference changes.
...
Async pipe: subscribe, display, cleanup. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. It manages subscription lifecycle. For Observables it handles emissions and cleanup
2. multiple async-pipe uses on a cold stream can create multiple subscriptions unless the value is aliased/shared.
...
Render `paymentState$ | async` into loading/error/content branches without a manual subscription that leaks after navigating away.
...
// template: @if (vm$ | async; as vm) { <app-payment [vm]="vm" /> }
```
...
**Use it when:** For declaratively rendering Observable/Promise values.

**Avoid or reconsider it when:** For commands/side effects that must execute imperatively or streams requiring orchestration before rendering.
...
`toSignal` converts an Observable to a signal in an injection context and also handles cleanup. Manual subscribe is for imperative side effects, not ordinary rendering.
...
- Multiple subscriptions causing duplicate HTTP
- async pipe inside every row
- ignoring null before first emission
- errors terminating stream
- calling observable factory in template.
...
A payment screen uses **What is the async pipe, and why is it useful**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** The async pipe subscribes to an Observable or Promise in a template, renders the latest value, marks the view for update, and automatically unsubscribes when the view is destroyed or the expression reference changes.
- **Remember:** Async pipe: subscribe, display, cleanup.
