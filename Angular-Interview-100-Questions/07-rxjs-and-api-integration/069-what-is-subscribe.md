...
`subscribe` starts an Observable execution/listener and supplies handlers for next, error, and completion. It creates a resource whose ownership and teardown must be understood. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Subscription is the execution boundary. Ask who owns it and who stops it.**

## 1. What problem does it solve?

`subscribe` starts an Observable execution/listener and supplies handlers for next, error, and completion. It creates a resource whose ownership and teardown must be understood.
...
Subscription is the execution boundary. Ask who owns it and who stops it. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. A cold observable creates work per subscription
2. a hot observable attaches to ongoing work. Operators are lazy until subscribe. Angular template async pipe and signal interop often subscribe for you.
...
Subscribe imperatively to submit a transfer because it is a command side effect, while render-only account data uses async pipe/toSignal.
...
```ts
submit(){this.api.create(this.request()).pipe(take(1)).subscribe({next:p=>this.done.set(p),error:e=>this.error.set(toMessage(e))})}
```
...
**Use it when:** At imperative edges: user commands, integration callbacks, or application startup effects.

**Avoid or reconsider it when:** For ordinary template rendering or derived state where async pipe/signals are clearer.
...
Use `tap` for controlled side effects inside a pipeline; use subscribe at the application edge. Avoid subscribe-inside-subscribe; flatten with switch/merge/concat/exhaust map.
...
- Ignoring error
- returning subscription from service
- nested subscribe
- duplicate subscriptions
- manual subscribe only to copy data into another field.
...
- **Definition:** `subscribe` starts an Observable execution/listener and supplies handlers for next, error, and completion. It creates a resource whose ownership and teardown must be understood.
- **Remember:** Subscription is the execution boundary. Ask who owns it and who stops it.
