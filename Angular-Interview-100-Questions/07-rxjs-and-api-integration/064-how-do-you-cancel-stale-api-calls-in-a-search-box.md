# How do you cancel stale API calls in a search box?
...
Cancel stale search calls with a value-change stream using normalization, debounce, distinct comparison, and `switchMap` to the HTTP search. In production, I choose it based on ownership, lifetime, testability, and failure behavior—not just because the syntax is convenient.

## Memory anchor

> **Search is latest-wins, so switch.**

## 1. What problem does it solve?

Cancel stale search calls with a value-change stream using normalization, debounce, distinct comparison, and `switchMap` to the HTTP search.
...
Search is latest-wins, so switch. Imagine the Angular screen as a set of small responsibilities. This concept tells you who owns a value or action, when it changes, and how the next part of the screen learns about it.

## 3. How it works internally

1. A new search term unsubscribes from the previous inner request. Angular HttpClient can abort the client request
2. switchMap also prevents an older response from updating the current stream.
...
Beneficiary search waits 300 ms, ignores unchanged trimmed terms, returns empty for short input, cancels stale calls, and displays a controlled error state.
...
```ts
results$=control.valueChanges.pipe(map(x=>x.trim()),filter(x=>x.length>=3),debounceTime(300),distinctUntilChanged(),switchMap(q=>api.search(q).pipe(catchError(()=>of([])))));
```
...
**Use it when:** For typeahead/search and other latest-state reads.

**Avoid or reconsider it when:** For writes or work where every emission must complete.
...
Debounce reduces frequency; distinct prevents repeats; switchMap enforces latest-wins; server rate limiting still protects the backend.
...
- Nested subscribe
- missing debounce
- error outside inner stream kills future searches
- loading flag race
- server performs expensive work despite client abort.
...
A payment screen uses **How do you cancel stale API calls in a search box**, works during the happy path, but becomes inconsistent after refresh, rapid clicks, or navigation. How would you reproduce the issue, identify the state owner and lifetime, decide whether the fix belongs in the component/service/router/RxJS pipeline/API, and prove the fix with tests and telemetry?
...
- **Definition:** Cancel stale search calls with a value-change stream using normalization, debounce, distinct comparison, and `switchMap` to the HTTP search.
- **Remember:** Search is latest-wins, so switch.
